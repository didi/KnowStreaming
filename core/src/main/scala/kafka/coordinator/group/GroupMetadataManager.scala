...
import org.apache.kafka.common.requests.{FetchResponse, OffsetCommitRequest, OffsetFetchResponse}
...
class GroupMetadataManager(brokerId: Int,
  ...
  def isPartitionLoading(partition: Int) = inLock(partitionLock) { loadingPartitions.contains(partition) || loadingRemotePartitions.contains(partition) }
  ...
  def storeGroup(group: GroupMetadata,
                 ...
                 mirrorChain: Option[String] = None): Unit = {
    getMagic(partitionFor(group.groupId)) match {
      case Some(magicValue) =>
        ...
        val headers = mirrorChain.map(chain => new Header {
          override def key(): String = GroupMetadataManager.RECORD_HEAD_DIDI_HA_MIRROR_CHAIN
          override def value(): Array[Byte] = chain.getBytes
        }).toArray
  ...
  private[group] def loadGroupsAndOffsets(topicPartition: TopicPartition, onGroupLoaded: GroupMetadata => Unit): Unit = {
    try {
      ...
      inLock(partitionLock) {
        try {
          completePendingMirrorGroupsAndOffsets(topicPartition)
        } catch {
  ...
  def cleanupGroupMetadata(groups: Iterable[GroupMetadata],
                           ...
                           mirrorChain: Option[String] = None): Int = {
        ...
        val timestamp = time.milliseconds()
          ...
          replicaManager.nonOfflinePartition(appendPartition).foreach { partition =>
            ...
            removedOffsets.foreach { case (topicPartition, offsetAndMetadata) =>
              ...
              tombstones += new SimpleRecord(timestamp, commitKey, null, headers)
            }
            ...
            if (groupIsDead && groupMetadataCache.remove(groupId, group) && generation > 0) {
              ...
              tombstones += new SimpleRecord(timestamp, groupMetadataKey, null, headers)
  ...
  private val pendingMirrorGroupsByPartitions = mutable.Map[TopicPartition, mutable.Map[String, (String, Long, Option[GroupMetadata])]]()
  private val pendingMirrorOffsetsByPartitions = mutable.Map[TopicPartition, mutable.Map[GroupTopicPartition, (String, Long, OffsetAndMetadata)]]()

  private def completePendingMirrorGroupsAndOffsets(topicPartition: TopicPartition): Unit = {
    info(s"Do complete load pending mirror groups for $topicPartition")
    pendingMirrorGroupsByPartitions.get(topicPartition).foreach { pendingMirrorGroups =>
        pendingMirrorGroups.foreach {
          case (groupId, (mirrorChain, timestamp, groupOpt)) =>
            doLoadRemoteGroupMetadatas(groupId, groupOpt, mirrorChain, timestamp, initializing = true)
        }
        pendingMirrorGroups.clear()
      info(s"Complete load pending mirror groups for $topicPartition count ${pendingMirrorGroups.size}")
    }

    info(s"Do complete load pending mirror group offsets for $topicPartition")
    pendingMirrorOffsetsByPartitions.get(topicPartition).foreach { pendingMirrorOffsets =>
        pendingMirrorOffsets.foreach {
          case (groupTopicPartition, (mirrorChain, timestamp, offsetAndMetadata)) =>
            doLoadRemoteGroupOffsets(groupTopicPartition, offsetAndMetadata, mirrorChain, timestamp, initializing = true)
        }
        pendingMirrorOffsets.clear()
      info(s"Complete load pending mirror group offsets for $topicPartition count ${pendingMirrorOffsets.size}")
    }
  }

  def cleanRemotePartitionLoadingState(topicPartition: TopicPartition): Unit = {
    inLock(partitionLock) {
      if (loadingRemotePartitions.remove(topicPartition.partition()))
        info(s"Clean mirror group remote loading partition ${topicPartition.partition()}")
    }
  }
  ...
  def doMirrorGroupsAndOffsets(topicPartition: TopicPartition,
    ...
    while (!ownedPartitions.contains(topicPartition.partition) &&
      ...
      info(s"Wait offset partiton $topicPartition begin loading, cost ${time.milliseconds() - nowMs} ms")
      time.sleep(100)
    }
    ...
    val loadedOffsets = mutable.Map[GroupTopicPartition, (String, Long, OffsetAndMetadata)]()
    val loadedGroups = mutable.Map[String, (String, Long, Option[GroupMetadata])]()

    val pendingMirrorGroups = pendingMirrorGroupsByPartitions
      .getOrElseUpdate(topicPartition, mutable.Map[String, (String, Long, Option[GroupMetadata])]())
    val pendingMirrorOffsets = pendingMirrorOffsetsByPartitions
      .getOrElseUpdate(topicPartition, mutable.Map[GroupTopicPartition, (String, Long, OffsetAndMetadata)]())
    ...
    memRecords.batches.asScala.foreach { batch =>
      ...
      lastOffset = Some(batch.lastOffset)
      ...
      if (batch.isControlBatch) {
        ...
        for (record <- batch.asScala) {
          require(record.hasKey, "Group metadata/offset entry key should not be null")
          val mirrorChain = record.headers().find(_.key().equals(GroupMetadataManager.RECORD_HEAD_DIDI_HA_MIRROR_CHAIN))
            .map(v => new String(v.value()))
            .getOrElse(s"[$remoteClusterId]")
          val newMirrorChain = s"$mirrorChain[$localClusterId]"
          if (!mirrorChain.contains(s"[$localClusterId]")) {
            ...
            GroupMetadataManager.readMessageKey(record.key) match {
              case offsetKey: OffsetKey =>
                ...
                if (replicaManager.isMirrorTopicByLocal(groupTopicPartition.topicPartition.topic())) {
                  // use remote topic leaderEpoch
                  val offsetAndMetadata = GroupMetadataManager.readOffsetMessageValue(record.value)

                  inLock(partitionLock) {
                    if (loadingPartitions.contains(partitionFor(groupId)) || pendingMirrorOffsets.contains(groupTopicPartition)) {
                      pendingMirrorOffsets += groupTopicPartition -> (newMirrorChain, timestamp, offsetAndMetadata)
                      info(s"Add pending load offset of $topicPartition for group topic partition $groupTopicPartition")
                    } else {
                      loadedOffsets += groupTopicPartition -> (newMirrorChain, timestamp, offsetAndMetadata)
                    }
                  }
                }
              ...
              case groupMetadataKey: GroupMetadataKey =>
                val groupId = groupMetadataKey.key
                info(s"Prepare to load[${replicaManager.isMirrorSyncGroupMetadata()}] metadata for group $groupId ...")
                // load group metadata
                if (replicaManager.isMirrorSyncGroupMetadata()) {
                  val groupOpt = Option(GroupMetadataManager.readGroupMessageValue(groupId, record.value, time))
                  val protocolType = groupOpt.flatMap(_.protocolType)
                  val existsGroup = getGroup(groupId).nonEmpty
                  if (existsGroup || protocolType.isEmpty || ConsumerProtocol.PROTOCOL_TYPE.equals(protocolType.get)) {
                    val members = groupOpt.map(_.allMemberMetadata).getOrElse(List.empty)
                    val isMirrorGroup = members.flatMap { member =>
                      if (member.assignment.length > 0) {
                        val assignment = ConsumerProtocol.deserializeAssignment(ByteBuffer.wrap(member.assignment))
                        assignment.partitions().asScala
                      } else {
                        None
                      }
                    }.forall(p => replicaManager.isMirrorTopicByLocal(p.topic()))

                    if (isMirrorGroup || (existsGroup && members.isEmpty)) {
                      inLock(partitionLock) {
                        if (loadingPartitions.contains(partitionFor(groupId)) || pendingMirrorGroups.contains(groupId)) inLock(partitionLock) {
                          pendingMirrorGroups += groupId -> (newMirrorChain, timestamp, groupOpt)
                          info(s"Add pending load metadata of $topicPartition for group $groupId")
                        } else {
                          loadedGroups += groupId -> (newMirrorChain, timestamp, groupOpt)
                          info(s"Add load metadata of $topicPartition for group $groupId")
                        }
                      }
                    }
                  }
    ...
    loadedGroups.foreach {
      case (groupId, (mirrorChain, timestamp, groupOpt)) =>
        doLoadRemoteGroupMetadatas(groupId, groupOpt, mirrorChain, timestamp, loadingRemotePartitions.contains(topicPartition.partition()))
    }

    loadedOffsets.foreach {
      case (groupTopicPartition, (mirrorChain, timestamp, offsetAndMetadata)) =>
        doLoadRemoteGroupOffsets(groupTopicPartition, offsetAndMetadata, mirrorChain, timestamp, loadingRemotePartitions.contains(topicPartition.partition()))
    }


    lastOffset.foreach { offset =>
      val lastLag = partitionData.highWatermark - offset - 1
      // info(s"Loaded mirror group remote partition $topicPartition fetchOffset $fetchOffset lastOffset $offset highWatermark ${partitionData.highWatermark} lastLag $lastLag sizeInBytes ${memRecords.sizeInBytes()} firstBatch ${memRecords.firstBatch()} maxTimestamp ${Option(memRecords.firstBatch()).map(_.maxTimestamp())}")
      if (lastLag <= 0 && loadingRemotePartitions.contains(topicPartition.partition())) inLock(partitionLock) {
  ...
  private def doLoadRemoteGroupMetadatas(groupId: String, groupOpt: Option[GroupMetadata], mirrorChain: String, timestamp: Long, initializing: Boolean = false): Unit = {
    trace(s"Mirror group metadata for group $groupId metadata $groupOpt state timestamp ${groupOpt.map(_.currentStateTimestamp)}")
    val currentGroupOpt = getGroup(groupId)
    groupOpt match {
      case Some(groupMetadata: GroupMetadata) =>
        if (!currentGroupOpt.exists(_.currentStateTimestamp.getOrElse(-1L) > groupMetadata.currentStateTimestamp.getOrElse(timestamp))) {
          info(s"Do load mirror group metadata for group $groupId metadata $groupOpt state timestamp ${groupOpt.map(_.currentStateTimestamp)}")
          ...
          currentGroupOpt.foreach {
            currentGroup => {
              ...
              groupMetadata.initializeOffsets(initialOffsets, Map.empty)
            }
          }
          groupMetadataCache.put(groupMetadata.groupId, groupMetadata)
          storeGroup(groupMetadata, groupMetadata.allMemberMetadata.map(m => m.memberId -> m.assignment).toMap, error => {
      ...
      case None =>
        if (!currentGroupOpt.exists(_.currentStateTimestamp.getOrElse(-1L) > timestamp))
          cleanupGroupMetadata(currentGroupOpt.toSeq, _.removeAllOffsets(), Some(mirrorChain))
    }
  }

  private def doLoadRemoteGroupOffsets(groupTopicPartition: GroupTopicPartition, offsetAndMetadata: OffsetAndMetadata,
                                       mirrorChain: String, timestamp: Long, initializing: Boolean = false): Unit = {
    trace(s"Mirror group offset for partition $groupTopicPartition metadata $offsetAndMetadata commit timestamp ${Option(offsetAndMetadata).map(_.commitTimestamp)}")
    val groupId = groupTopicPartition.group
    ...
    if (offsetAndMetadata == null) {
      if (!group.offset(groupTopicPartition.topicPartition)
        .exists(_.commitTimestamp > timestamp))
        cleanupGroupMetadata(Seq(group), _.removeOffsets(Seq(groupTopicPartition.topicPartition)), Some(mirrorChain))
    } else {
      // do when not exists or less then pending commitTimestamp
      if (!group.offset(groupTopicPartition.topicPartition)
        .exists(_.commitTimestamp > offsetAndMetadata.commitTimestamp)) {
        storeOffsets(group, "mirror-group", immutable.Map(groupTopicPartition.topicPartition -> offsetAndMetadata), commitStatus => {
  ...
  private def analyzeAndValidateMirrorRecords(records: MemoryRecords): LogAppendInfo = {
    ...
    LogAppendInfo(firstOffset, lastOffset, maxTimestamp, offsetOfMaxTimestamp, RecordBatch.NO_TIMESTAMP, -1,
      RecordConversionStats.EMPTY, NoCompressionCodec, NoCompressionCodec, shallowMessageCount, validBytesCount, monotonic, lastOffsetOfFirstBatch)
  }
}
...
object GroupMetadataManager {
  ...
  val RECORD_HEAD_DIDI_HA_MIRROR_CHAIN = "didi.kafka.ha.mirror.chain"
