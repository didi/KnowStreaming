...
class GroupCoordinator(val brokerId: Int,
      ...
      if (group.is(Dead)) {
        ...
        if (group.hasStaticMember(groupInstanceId)) {
          ...
          debug(s"Dynamic member with unknown member id joins group ${group.groupId} in " +
            ...
            s"$newMemberId(${RequestChannel.currentSession.getHostAddress}) and request the member to rejoin with this id.")
          ...
          info(s"${if (groupInstanceId.isDefined) "Static" else "Dynamic"} Member with unknown member id joins group ${group.groupId} in " +
            s"${group.currentState} state. Created a new member id " +
            s"$newMemberId(${RequestChannel.currentSession.getHostAddress}) for this member and add to the group.")
        ...
        group.currentState match {
          ...
          case CompletingRebalance =>
            ...
            if (group.isLeader(memberId)) {
              info(s"Assignment received from leader for group ${group.groupId} of member ${memberId} for generation ${group.generationId}")
  ...
  def handleRemoveHeartbeatForMember(groupId: String,
                                     memberId: String,
                                     groupInstanceId: Option[String],
                                     generationId: Int): Unit = {
  ...
  private def addMemberAndRebalance(rebalanceTimeoutMs: Int,
    ...
    val appId = RequestChannel.currentSession.getUsername
    val member = new MemberMetadata(memberId, group.groupId, groupInstanceId,
      appId + "." + clientId, clientHost, rebalanceTimeoutMs,
  ...
  private def updateStaticMemberAndRebalance(group: GroupMetadata,
    ...
    info(s"Static member $groupInstanceId of group ${group.groupId} with unknown member id rejoins, " +
      s"assigning new member id $newMemberId(${RequestChannel.currentSession.getHostAddress}), while " +
    ...
    group.inLock {
      ...
      if (group.is(Dead)) {
        ...
        if (group.is(Empty)) {
          ...
          info(s"Stabilized group ${group.groupId} generation ${group.generationId}, all members ${group.allMembers}. " +
            s"(${Topic.GROUP_METADATA_TOPIC_NAME}-${partitionFor(group.groupId)})")
