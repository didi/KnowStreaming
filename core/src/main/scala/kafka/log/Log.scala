...
import com.didichuxing.datachannel.kafka.server.{FetchInfo, OSUtil}
...
case class LogAppendInfo(var firstOffset: Option[Long],
                         ...
                         var diskName: String = OSUtil.DEFAULT_DISKNAME) {
  /**
...
class Log(@volatile var dir: File,
  ...
  newGauge(LogMetricNames.Directory, () => dir.getParent, tags)
  ...
  def appendAsLeader(records: MemoryRecords,
    ...
    // 如果数据来自mirror并且消息格式一致不做offset重分配，但如果消息格式不一样走重分配逻辑并做消息格式转换
    // val assignOffsets = !(origin == AppendOrigin.Mirror && records.hasMatchingMagic(config.messageFormatVersion.recordVersion.value))
    val assignOffsets = (origin != AppendOrigin.Mirror) || !records.hasMatchingMagic(config.messageFormatVersion.recordVersion.value)
    append(records, origin, interBrokerProtocolVersion, assignOffsets, leaderEpoch)
  }
  ...
  private def append(records: MemoryRecords,
    ...
    maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
      ...
      Option(KafkaExMetrics.produceRequestMetrics(topicPartition.topic)).foreach(_.markCompress(appendInfo.sourceCodec.name))
          ...
          if (validateAndOffsetAssignResult.messageSizeMaybeChanged) {
            for (batch <- validRecords.batches.asScala) {
              if (batch.sizeInBytes > config.maxMessageSize) {
                ...
                Option(KafkaExMetrics.appIdTopicMetrics(topicPartition.topic)).foreach(_.markBytesRejectedRate(records.sizeInBytes))
...
//            // mirror topic rewrite leaderEpoch // do not change, same as source
...
//            }
        ...
        var diskName = segment.log.getDiskname;
        if (diskName == null) {
          diskName = OSUtil.instance().getDiskName(segment.log.file().getAbsolutePath);
          segment.log.setDiskname(diskName);
        }
        appendInfo.diskName = segment.log.getDiskname;
  ...
  private def analyzeAndValidateRecords(records: MemoryRecords, origin: AppendOrigin): LogAppendInfo = {
    ...
    for (batch <- records.batches.asScala) {
      ...
      if (batchSize > config.maxMessageSize) {
        brokerTopicStats.topicStats(topicPartition.topic).bytesRejectedRate.mark(records.sizeInBytes)
        Option(KafkaExMetrics.appIdTopicMetrics(topicPartition.topic)).
          foreach(_.markBytesRejectedRate(records.sizeInBytes))
        brokerTopicStats.allTopicsStats.bytesRejectedRate.mark(records.sizeInBytes)
  ...
  private def emptyFetchDataInfo(fetchOffsetMetadata: LogOffsetMetadata,
    ...
    val fetchDataInfo = FetchDataInfo(fetchOffsetMetadata,
      MemoryRecords.EMPTY,
      firstEntryIncomplete = false,
      abortedTransactions = abortedTransactions)
    val filename = segments.lastEntry.getValue.log.file.getAbsolutePath;
    val fetchInfo  = new FetchInfo(filename, nextOffsetMetadata, 0);
    fetchInfo.setPolicy(FetchInfo.ReadPolicy.Empty)
    fetchInfo.setHotdata(true)
    fetchDataInfo.fetchInfo = fetchInfo
    fetchDataInfo
  }
    ...
    maybeHandleIOException(s"Exception while reading from $topicPartition in dir ${dir.getParent}") {
      ...
      while (segmentEntry != null) {
        ...
        if (fetchInfo == null) {
          ...
          val filename = segmentEntry.getValue.log.file.getAbsolutePath;
          val fi = new FetchInfo(filename, fetchInfo.fetchOffsetMetadata, maxLength);
          val active = segmentEntry == segments.lastEntry;
          fi.setHotdata(active)
          var diskName = segmentEntry.getValue.log.getDiskname;
          if (diskName == null) {
            diskName = OSUtil.instance().getDiskName(filename);
            segmentEntry.getValue.log.setDiskname(diskName);
          }
          fi.setDiskname(diskName);
      ...
      val fetchInfo  = new FetchInfo("unkonw", LogOffsetMetadata.UnknownOffsetMetadata, maxLength);
      fetchInfo.setPolicy(FetchInfo.ReadPolicy.Empty)
      fetchInfo.setHotdata(true)
  ...
  private[log] def removeLogMetrics(): Unit = {
    ...
    removeMetric(LogMetricNames.Directory, tags)
  }
...
object LogMetricNames {
  ...
  def allMetricNames: List[String] = {
    List(NumLogSegments, LogStartOffset, LogEndOffset, Size, Directory)
  }
}
