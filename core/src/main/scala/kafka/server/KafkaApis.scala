...
import com.didichuxing.datachannel.kafka.security.authorizer.{AccessStatus, AccessStatusAndTimestamp, Operation}
...
import kafka.network.RequestChannel.{EndThrottlingResponse, Response}
...
import org.apache.kafka.common.internals.Topic.{GROUP_METADATA_TOPIC_NAME, MIRROR_STATE_TOPIC_NAME, TRANSACTION_STATE_TOPIC_NAME, isInternal}
...
class KafkaApis(val requestChannel: RequestChannel,
                ...
                val mirrorCoordinatorOpt: Option[MirrorCoordinator] = None,
                val forwardingManager: Option[ForwardingManager] = None) extends Logging {
  ...
  def handle(request: RequestChannel.Request): Unit = {
    try {
      ...
      request.header.apiKey match {
        case ApiKeys.PRODUCE => maybeBreakHAUser(request, breakError = Errors.UNKNOWN_TOPIC_OR_PARTITION, sendAndCloseConnection = true, handleProduceRequest)
        case ApiKeys.FETCH => maybeBreakHAUser(request, createResponse = createFetchErrorResponse, sendAndCloseConnection = true, handleFetchRequest)
        case ApiKeys.LIST_OFFSETS => handleListOffsetRequest(request)
        case ApiKeys.METADATA => maybeForwardToHACluster(request, handleTopicMetadataRequest)
        case ApiKeys.LEADER_AND_ISR => handleLeaderAndIsrRequest(request)
        case ApiKeys.STOP_REPLICA => handleStopReplicaRequest(request)
        case ApiKeys.UPDATE_METADATA => handleUpdateMetadataRequest(request)
        case ApiKeys.CONTROLLED_SHUTDOWN => handleControlledShutdownRequest(request)
        case ApiKeys.OFFSET_COMMIT => maybeBreakHAUser(request, breakError = Errors.COORDINATOR_NOT_AVAILABLE, sendAndCloseConnection = false, handleOffsetCommitRequest)
        case ApiKeys.OFFSET_FETCH => maybeBreakHAUser(request, breakError = Errors.NOT_COORDINATOR, sendAndCloseConnection = true, handleOffsetFetchRequest)
        case ApiKeys.FIND_COORDINATOR => maybeForwardToHACluster(request, handleFindCoordinatorRequest)
        case ApiKeys.JOIN_GROUP => maybeBreakHAUser(request, breakError = Errors.COORDINATOR_NOT_AVAILABLE, sendAndCloseConnection = false, handleJoinGroupRequest)
        case ApiKeys.HEARTBEAT => maybeBreakHAUser(request, createError = createHeartbeatError, sendAndCloseConnection = false, handleHeartbeatRequest)
        case ApiKeys.LEAVE_GROUP => handleLeaveGroupRequest(request)
        case ApiKeys.SYNC_GROUP => handleSyncGroupRequest(request)
        case ApiKeys.DESCRIBE_GROUPS => handleDescribeGroupRequest(request)
        case ApiKeys.LIST_GROUPS => handleListGroupsRequest(request)
        case ApiKeys.SASL_HANDSHAKE => handleSaslHandshakeRequest(request)
        case ApiKeys.API_VERSIONS => maybeForwardToHACluster(request, handleApiVersionsRequest)
        case ApiKeys.CREATE_TOPICS => handleCreateTopicsRequest(request)
        case ApiKeys.DELETE_TOPICS => handleDeleteTopicsRequest(request)
        case ApiKeys.DELETE_RECORDS => handleDeleteRecordsRequest(request)
        case ApiKeys.INIT_PRODUCER_ID => maybeForwardToHACluster(request, handleInitProducerIdRequest)
        case ApiKeys.OFFSET_FOR_LEADER_EPOCH => maybeBreakHAUser(request, breakError = Errors.UNKNOWN_TOPIC_OR_PARTITION, sendAndCloseConnection = true, handleOffsetForLeaderEpochRequest)
  ...
  private def activeClusterForTopic(request: RequestChannel.Request): Option[String] = {
    if (!request.header.clientId().startsWith("P#")) {
      return None
    }
    val topicNameOpt = request.header.apiKey match {
      case ApiKeys.PRODUCE =>
        val produceRequest = request.body[ProduceRequest]
        produceRequest.partitionRecordsOrFail
          .asScala.toSeq.map(_._1.topic)
          .headOption
      case ApiKeys.METADATA =>
        val metadataRequest = request.body[MetadataRequest]
        if (metadataRequest.isAllTopics)
          metadataCache.getAllTopics().headOption
        else
          metadataRequest.topics.asScala.headOption
      case _ => None
    }

    // 对于生产者访问的请求，如果是高可用备Topic返回主集群，如果高可用主Topic或者普通Topic返回当前集群
    topicNameOpt match {
      case Some(topicName) =>
        Some(
          replicaManager.allMirrorTopicsByLocal()
          .get(topicName)
          .map(_.remoteCluster)
          .getOrElse(clusterId)
        )
...
//    topicNameOpt
//      .flatMap(replicaManager.allMirrorTopicsByLocal().get)
//      .map(_.remoteCluster)
  }

  def createFetchErrorResponse(request: RequestChannel.Request, activeCluster: String, throttleTimeMs: Int): AbstractResponse = {
    ...
    val erroneous = new util.LinkedHashMap[TopicPartition, FetchResponse.PartitionData[Records]]
    fetchRequest.fetchData().asScala.foreach { case (topicPartition, _) =>
      erroneous.put(topicPartition, errorResponse(Errors.UNKNOWN_TOPIC_OR_PARTITION))
    }
    // to remove session in cache
    // val sessionlessFetchContext =
    fetchManager.newContext(
      new JFetchMetadata(fetchRequest.metadata.sessionId(), JFetchMetadata.FINAL_EPOCH),
      fetchRequest.fetchData,
      fetchRequest.toForget,
      fetchRequest.isFromFollower)

    // sessionlessFetchContext.updateAndGenerateResponseData(erroneous)
    // the same use
    new FetchResponse(Errors.NONE, erroneous, throttleTimeMs, JFetchMetadata.INVALID_SESSION_ID)
  }

  def createHeartbeatError(request: RequestChannel.Request, activeCluster: String): Errors = {
    val heartbeatRequest = request.body[HeartbeatRequest]
    groupCoordinator.handleRemoveHeartbeatForMember(
    ...
    activeCluster match {
      case "None" if !replicaManager.isMirrorSyncGroupMetadata() => Errors.REBALANCE_IN_PROGRESS
      case _ => Errors.COORDINATOR_NOT_AVAILABLE
    }
  }

  def maybeBreakHAUser(request: RequestChannel.Request,
                       breakError: Errors,
                       sendAndCloseConnection: Boolean,
                       handler: RequestChannel.Request => Unit): Unit = {
    maybeBreakHAUser(request, (_, _) => breakError, sendAndCloseConnection, handler)
  }

  def maybeBreakHAUser(request: RequestChannel.Request,
                       createError: (RequestChannel.Request, String) => Errors,
                       sendAndCloseConnection: Boolean,
                       handler: RequestChannel.Request => Unit): Unit = {
    def createErrorResponse(request: RequestChannel.Request, activeCluster: String, throttleTimeMs: Int): AbstractResponse = {
      val requestBody = request.body[AbstractRequest]
      requestBody.getErrorResponse(throttleTimeMs, createError(request, activeCluster).exception)
    }
    maybeBreakHAUser(request, createResponse = createErrorResponse, sendAndCloseConnection, handler)
  }

  def maybeBreakHAUser(request: RequestChannel.Request,
                       createResponse: (RequestChannel.Request, String, Int) => AbstractResponse,
                       sendAndCloseConnection: Boolean,
                       handler: RequestChannel.Request => Unit): Unit = {
    val requestApiKey = request.header.apiKey
    // Optimization
    if (requestApiKey == ApiKeys.FETCH && request.body[FetchRequest].isFromFollower) {
      handler(request)
      return
    }

    // DiDi HA
    val activeCluster = UserConfigManager.activeCluster(request.context, clusterId, () => activeClusterForTopic(request).asJava)
    if (activeCluster == clusterId) {
      handler(request)
    } else {
      if (requestApiKey == ApiKeys.OFFSET_COMMIT && activeCluster == "None") {
        handler(request)
      } else {
        info(s"[$requestApiKey] - ${request.header.correlationId()} User ${request.context.principal().getName} client-id ${request.header.clientId()} is binding to cluster $activeCluster, will reconnect to new cluster.")
        if (sendAndCloseConnection && activeCluster != "None") {
          val throttleTimeMs = 500
          info(s"[$requestApiKey] - ${request.header.correlationId()} Begin sent response...")
          def throttlingCallbackAndAddDelayToCloseConnection(response: RequestChannel.Response): Unit = {
            requestChannel.sendResponse(response)
            response match {
              case _: EndThrottlingResponse =>
                info(s"[$requestApiKey] - ${request.header.correlationId()} Complete sent response.")
                // After response add delay to close connection
                quotas.request.throttle(request, throttleTimeMs * 3 + 10, throttlingCallbackToCloseConnection)
              case _ =>
            }
          }
          def throttlingCallbackToCloseConnection(response: RequestChannel.Response): Unit = {
            // don't send response only use delay queue // requestChannel.sendResponse(response)
            response match {
              case _: EndThrottlingResponse =>
                info(s"[$requestApiKey] - ${request.header.correlationId()} Sent close response.")
                requestChannel.sendResponse(new RequestChannel.CloseConnectionResponse(request))
              case _ =>
            }
          }
          sendResponseWithFixThrottle(request, Some(throttleTimeMs => createResponse(request, activeCluster, throttleTimeMs)),
            throttlingCallbackAndAddDelayToCloseConnection, throttleTimeMs)
        } else {
          sendResponseWithFixThrottle(request, Some(throttleTimeMs => createResponse(request, activeCluster, throttleTimeMs)),
            sendResponse)
        }
  ...
  private def sendResponseWithFixThrottle(request: RequestChannel.Request,
                                          createResponseOpt: Option[Int => AbstractResponse],
                                          channelThrottlingCallback: Response => Unit,
                                          throttleTimeMs: Int = 800): Unit = {
    // also do get throttle time
    quotas.request.maybeRecordAndGetThrottleTimeMs(request)
    quotas.request.throttle(request, throttleTimeMs, channelThrottlingCallback)
    sendResponse(request, createResponseOpt.map(_.apply(throttleTimeMs)), None)
  }
  ...
  def handleUpdateMetadataRequest(request: RequestChannel.Request): Unit = {
    ...
    if (isBrokerEpochStale(updateMetadataRequest.brokerEpoch)) {
      ...
      // update quota
      val allTopics = updateMetadataRequest.partitionStates.asScala.map(_.topicName).toSet
      val deletedTopics = deletedPartitions.map(_.topic())
      val changedTopics = allTopics -- deletedTopics
      if (changedTopics.nonEmpty) {
        quotas.produce.updateTopicQuota(changedTopics)
        quotas.fetch.updateTopicQuota(changedTopics)
      }
  ...
  def handleProduceRequest(request: RequestChannel.Request): Unit = {
    ...
    produceRequest.partitionRecordsOrFail().asScala.foreach(entry => request.initTopicMetrics(entry._1.topic()))
    ...
    // set didi session access status
    authorizedTopics.foreach(topic =>
      request.session.kafkaSession.setAccessStatus(topic, Operation.Write, new AccessStatusAndTimestamp(AccessStatus.Allow, time.milliseconds()))
    )

    for ((topicPartition, memoryRecords) <- produceRequest.partitionRecordsOrFail.asScala) {
      request.topicRequestSizeInByes(topicPartition.topic()) += memoryRecords.sizeInBytes()
    ...
    val compositionBySize = produceRequest.partitionRecordsOrFail.asScala.
      map(kv => (kv._1, kv._2.sizeInBytes())).groupBy(_._1.topic()).map(kv => (kv._1, kv._2.values.sum))
    request.throttleId = quotas.produce.getThrottleId(request, compositionBySize)
    ...
    def sendResponseCallback(responseStatus: Map[TopicPartition, PartitionResponse]): Unit = {
      ...
      mergedResponseStatus.foreach { case (topicPartition, status) =>
        if (status.error != Errors.NONE) {
          ...
          Option(KafkaExMetrics.produceRequestMetrics(topicPartition.topic)).
            foreach(_.markError(status.error, status.error.exceptionName))
  ...
  def handleFetchRequest(request: RequestChannel.Request): Unit = {
    ...
    if (fetchRequest.isFromFollower) {
      ...
      // set didi session access status
      authorizedTopics.foreach(topic =>
        request.session.kafkaSession.setAccessStatus(topic, Operation.Read, new AccessStatusAndTimestamp(AccessStatus.Allow, time.milliseconds()))
      )
    ...
    def processResponseCallback(responsePartitionData: Seq[(TopicPartition, FetchPartitionData)]): Unit = {
      ...
      def createResponse(throttleTimeMs: Int): FetchResponse[BaseRecords] = {
        ...
        unconvertedFetchResponse.responseData.asScala.foreach { case (tp, unconvertedPartitionData) =>
          if (unconvertedPartitionData.error != Errors.NONE) {
            Option(KafkaExMetrics.fetchRequestMetrics(tp.topic)).
              foreach(_.markError(unconvertedPartitionData.error, unconvertedPartitionData.error.exceptionName))
        ...
        response.responseData.asScala.foreach { case (tp, data) =>
          request.topicResponseSizeInByes(tp.topic) += data.records.sizeInBytes()
      ...
      if (fetchRequest.isFromMirror) {
        ...
        val compositionBySize = partitions.asScala.
          map(kv => (kv._1, kv._2.records.sizeInBytes())).groupBy(_._1.topic()).map(kv => (kv._1, kv._2.values.sum))
        request.throttleId = quotas.fetch.getThrottleId(request, compositionBySize)
    ...
    interesting.foreach(entry => request.initTopicMetrics(entry._1.topic))
    erroneous.foreach(entry => request.initTopicMetrics(entry._1.topic))
  ...
  def replicationQuota(fetchRequest: FetchRequest): ReplicaQuota = {
    if (fetchRequest.isFromMirror)
      quotas.mirrorLeader
    else if (fetchRequest.isFromFollower) quotas.leader else UnboundedQuota
  }
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request): Unit = {
    ...
    var brokers = metadataCache.getAliveBrokers
    // request all topic return all brokers
    if (!metadataRequest.isAllTopics && topicMetadata.nonEmpty) {
      val neededNodeIds = topicMetadata.flatMap(_.partitionMetadata().asScala).flatMap(_.replicaIds.asScala).
        toSet.filter(v => metadataCache.getAliveBroker(v.intValue).nonEmpty)
      val needBrokers = neededNodeIds.map(v => metadataCache.getAliveBroker(v.intValue).get).toSeq
      if (needBrokers.size>=3)
        brokers = needBrokers
    }
    ...
    sendResponseMaybeThrottle(request, requestThrottleMs =>
       MetadataResponse.prepareResponse(
         requestThrottleMs,
         ClusterConfigManager.appendWithGatewayNodes(request.context, brokers.flatMap(_.getNode(request.context.listenerName)).asJava),
         clusterId,
  ...
  def handleDescribeAcls(request: RequestChannel.Request): Unit = {
    ...
    authorizer match {
      ...
      case Some(auth) =>
        ...
        auth.acls(filter).forEach(acl => returnedAcls.add(acl))
        sendResponseMaybeThrottle(request, requestThrottleMs =>
          new DescribeAclsResponse(new DescribeAclsResponseData()
            .setThrottleTimeMs(requestThrottleMs)
    ...
    if (conversionCount > 0) {
      request.header.apiKey match {
        case ApiKeys.PRODUCE =>
          ...
          Option(KafkaExMetrics.appIdTopicMetrics(tp.topic)).foreach(_.produceMessageConversionsRate.mark())
        ...
        case ApiKeys.FETCH =>
          ...
          Option(KafkaExMetrics.appIdTopicMetrics(tp.topic)).foreach(_.fetchMessageConversionsRate.mark())
          brokerTopicStats.allTopicsStats.fetchMessageConversionsRate.mark(conversionCount)
        case _ =>
          throw new IllegalStateException("Message conversion info is recorded only for Produce/Fetch requests")
      }
      request.topicMessageConversionsTime(tp.topic()) += conversionStats.conversionTimeNanos
      request.messageConversionsTimeNanos = conversionStats.conversionTimeNanos
    }
    request.topicTemporaryMemoryBytes(tp.topic()) += conversionStats.temporaryMemoryBytes
  ...
  private def maybeForwardToHACluster(request: RequestChannel.Request, handler: RequestChannel.Request => Unit): Unit = {
    def responseCallback(responseOpt: Option[AbstractResponse]): Unit = {
      responseOpt match {
        case Some(response) => sendForwardedResponse(request, response)
        case None =>
          info(s"The client connection will be closed due to ha cluster responded " +
            s"unsupported version exception during $request forwarding")
          requestChannel.closeConnection(request, Collections.emptyMap())
      }
    }

    passthroughPrincipal(request).foreach { kafkaPrincipal =>
      info(s"${request.header.apiKey} The principal name in the request is ${request.context.principal.getName} " +
        s"and the passthrough principal name resolved from the request header is ${kafkaPrincipal.getName}")
      request.context.principal = kafkaPrincipal
    }

    val activeCluster = UserConfigManager.activeCluster(request.context, clusterId, () => activeClusterForTopic(request).asJava)
    if (clusterId == activeCluster || activeCluster == "None") {
      handler(request)
    } else {
      info(s"${request.header.apiKey} The user ${request.context.principal.getName} client-id ${request.context.clientId} of request correlationId ${request.header.correlationId()} forwarding to ha cluster $activeCluster")
      forwardingManager.foreach(_.forwardRequest(activeCluster, request, responseCallback))
    }
  }

  def passthroughPrincipal(request: RequestChannel.Request): Option[KafkaPrincipal] = {
    request.header.data().unknownTaggedFields()
      .asScala.find(t => t.tag() == TAG_FORWARD_CLIENT_HEADER_PRINCIPAL_CODE)
      .map(t => EnvelopeUtils.deserialize(t.data()))
  }
