...
import com.didichuxing.datachannel.kafka.server.{DiskLoadProtector, FetchInfo, TopicPartitionAndReplica}
...
class ReplicaManager(val config: KafkaConfig,
  ...
  private val log4j = LoggerFactory.getLogger(classOf[ReplicaManager]);
  var diskLoadProtector: DiskLoadProtector = null
  ...
  def startup(): Unit = {
    ...
    scheduler.schedule("mirror-remote-metadata-thread", updateRemoteTopicMetadata _, period = 10 * 1000, unit = TimeUnit.MILLISECONDS)
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
    ...
    def processFailedRecord(topicPartition: TopicPartition, t: Throwable) = {
      ...
      Option(KafkaExMetrics.appIdTopicMetrics(topicPartition.topic)).foreach(_.markFailedProduceRequestsRate)
    ...
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      Option(KafkaExMetrics.appIdTopicMetrics(topicPartition.topic)).foreach(_.markTotalProduceRequestsRate())
      ...
      if (Topic.isInternal(topicPartition.topic) && !internalTopicsAllowed) {
        ...
        try {
          val partition = getPartitionOrException(topicPartition, expectLeader = true)
          // mirror can't be produce records
          if (isMirrorTopicByLocal(topicPartition.topic()) && origin != AppendOrigin.Mirror) {
            throw Errors.NOT_LEADER_FOR_PARTITION.exception(s"Topic ${topicPartition.topic()} is a mirror topic, can's be append records from producer.")
          }
          ...
          Option(KafkaExMetrics.appIdTopicMetrics(topicPartition.topic)).foreach(_.markBytesInRate(records.sizeInBytes))
          brokerTopicStats.allTopicsStats.bytesInRate.mark(records.sizeInBytes)
          brokerTopicStats.topicStats(topicPartition.topic).messagesInRate.mark(numAppendedMessages)
          Option(KafkaExMetrics.appIdTopicMetrics(topicPartition.topic)).foreach(_.markMessagesInRate(numAppendedMessages))
          brokerTopicStats.allTopicsStats.messagesInRate.mark(numAppendedMessages)
          val diskMetrics = KafkaExMetrics.diskTopicPartitionMetrics(info.diskName)
          diskMetrics.updateWriteMetrics(topicPartition, records.sizeInBytes, 0)
  ...
  def fetchMessages(timeout: Long,
    ...
    if (timeout <= 0 || fetchInfos.isEmpty || bytesReadable >= fetchMinBytes || errorReadingData || anyPartitionsNeedHwUpdate) {
      val fetchPartitionData = logReadResults.map { case (tp, result) =>
        val role = if (isFromFollower) if (isAddingReplica(tp, replicaId))  -1 else 1 else 0
        val diskMetics = KafkaExMetrics.diskTopicPartitionMetrics(result.info.fetchInfo.getDiskname)
        diskMetics.updateReadMetrics(tp, result.info.records.sizeInBytes, role, result.info.fetchInfo.isHotdata)
      ...
      val finishedRead = mutable.ArrayBuffer[(TopicPartition, FetchPartitionData)]()
      var delay = timeout
      log4j.trace("finished read {}{}{}", logReadResults, "", "");

      fetchInfos.foreach { case (topicPartition, partitionData) =>
        logReadResultMap.get(topicPartition).foreach(logReadResult => {
          logReadResult.info.fetchInfo.getPolicy match {
            case FetchInfo.ReadPolicy.Normal =>
              if (logReadResult.info.records.sizeInBytes() < logReadResult.readSize) {
                val fp = FetchPartitionStatus(logReadResult.info.fetchOffsetMetadata, partitionData)
                fetchPartitionStatus += ((topicPartition, fp))
              } else {
                finishedRead += ((topicPartition, FetchPartitionData(logReadResult.error, logReadResult.highWatermark,
                  logReadResult.leaderLogStartOffset, logReadResult.info.records, logReadResult.lastStableOffset,
                  logReadResult.info.abortedTransactions, logReadResult.preferredReadReplica,
                  isFromFollower && isAddingReplica(topicPartition, replicaId))))
              }
            case FetchInfo.ReadPolicy.Empty =>
              val fp = FetchPartitionStatus(logReadResult.info.fetchOffsetMetadata, partitionData)
              fetchPartitionStatus += ((topicPartition, fp))
            case FetchInfo.ReadPolicy.Evade=>
              delay = diskLoadProtector.getIoUtilFrequency
              finishedRead += ((topicPartition, FetchPartitionData(logReadResult.error, logReadResult.highWatermark,
                logReadResult.leaderLogStartOffset, logReadResult.info.records, logReadResult.lastStableOffset,
                logReadResult.info.abortedTransactions, logReadResult.preferredReadReplica,
                isFromFollower && isAddingReplica(topicPartition, replicaId))))
            case FetchInfo.ReadPolicy.Greedy =>
              finishedRead += ((topicPartition, FetchPartitionData(logReadResult.error, logReadResult.highWatermark,
                logReadResult.leaderLogStartOffset, logReadResult.info.records, logReadResult.lastStableOffset,
                logReadResult.info.abortedTransactions, logReadResult.preferredReadReplica,
                isFromFollower && isAddingReplica(topicPartition, replicaId))))
      ...
      val delayedFetch = new DelayedFetch(delay, fetchMetadata, this, quota, finishedRead, clientMetadata,
        responseCallback)

      // create a list of (topic, partition) pairs to use as keys for this delayed fetch operation
      val delayedFetchKeys = if (fetchPartitionStatus.isEmpty)
          Seq(TopicPartitionOperationKey.WaitKey)
        else
          fetchPartitionStatus.map { case (tp, _) => TopicPartitionOperationKey(tp) }
  ...
  def readFromLocalLog(replicaId: Int,
    ...
    def read(tp: TopicPartition, fetchInfo: PartitionData, limitBytes: Int, minOneMessage: Boolean): LogReadResult = {
      ...
      Option(KafkaExMetrics.appIdTopicMetrics(tp.topic)).foreach(_.markTotalFetchRequestsRate())
      ...
      try {
        ...
        if (preferredReadReplica.isDefined) {
          ...
          var readInfo: LogReadInfo = partition.readRecords(
          ...
          readInfo.fetchedData.fetchInfo.setTopicPartitionAndReplica(new TopicPartitionAndReplica(tp, replicaId))
          readInfo.fetchedData.fetchInfo.setLag(readInfo.logEndOffset - fetchInfo.fetchOffset)

          diskLoadProtector.checkRead(readInfo.fetchedData.fetchInfo)
          log4j.trace("Disk load protector check:{}", readInfo.fetchedData.fetchInfo, "", "")
          readInfo =  readInfo.fetchedData.fetchInfo.getPolicy match {
            case FetchInfo.ReadPolicy.Normal =>
              readInfo
            case FetchInfo.ReadPolicy.Empty=>
              readInfo
            case FetchInfo.ReadPolicy.Evade =>
              //disk protector return status busy. it should return empty
              LogReadInfo(FetchDataInfo(readInfo.fetchedData.fetchOffsetMetadata,
                MemoryRecords.EMPTY, readInfo.fetchedData.fetchInfo),
                readInfo.highWatermark, readInfo.logStartOffset, readInfo.logEndOffset,
                readInfo.lastStableOffset
              )
            case FetchInfo.ReadPolicy.Greedy =>
              //disk protector return high priory topic partition. it should change fetch size.
              val size = math.min(readInfo.fetchedData.fetchInfo.getLength, limitBytes)
              val newReadInfo = partition.readRecords(
                fetchOffset = fetchInfo.fetchOffset,
                currentLeaderEpoch = fetchInfo.currentLeaderEpoch,
                maxBytes = size,
                fetchIsolation = fetchIsolation,
                fetchOnlyFromLeader = fetchOnlyFromLeader,
                minOneMessage = minOneMessage)
              newReadInfo.fetchedData.fetchInfo.setTopicPartitionAndReplica(new TopicPartitionAndReplica(tp, replicaId));
              newReadInfo.fetchedData.fetchInfo.setLag(readInfo.logEndOffset - fetchInfo.fetchOffset);
              newReadInfo
          }
        ...
        case e: Throwable =>
          ...
          Option(KafkaExMetrics.appIdTopicMetrics(tp.topic)).foreach(_.markFailedFetchRequestsRate)
  ...
  def shouldLeaderThrottle(quota: ReplicaQuota, topicPartition: TopicPartition, replicaId: Int): Boolean = {
    if (replicaId >= FetchRequest.MIRROR_START_REPLICA_ID) {
      return quota.isQuotaExceeded && !Topic.GROUP_METADATA_TOPIC_NAME.equals(topicPartition.topic())
    }
  ...
  def maybeUpdateMetadataCache(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest) : Seq[TopicPartition] =  {
    replicaStateChangeLock synchronized {
      if (updateMetadataRequest.controllerEpoch < controllerEpoch) {
        ...
        metadataCache.getMaintainTopics.foreach {
          tpAndReplica =>
            diskLoadProtector.addMaintainTopicPartitions(tpAndReplica);
        }
        try {
          // update mirror unknown topic
          updateMetadataRequest.partitionStates().asScala
            .filter(s => s.leader() != LeaderAndIsr.LeaderDuringDelete)
            .map(_.topicName()).toSet
            .filter(mirrorTopicsByLocal.contains)
            .foreach(removeUnknownLocalMirrorTopic)

          deletedPartitions.map(_.topic()).toSet[String]
            .filterNot(metadataCache.contains)
            .filter(mirrorTopicsByLocal.contains)
            .foreach(addUnknownLocalMirrorTopic)
        } catch {
          case e: Throwable =>
            error("Error update mirror unknown topic list", e)
        }
        ...
        leaderAndIsrRequest.partitionStates.asScala.foreach { partitionState =>
          ...
          partitionOpt.foreach { partition =>
            ...
            if (requestLeaderEpoch > currentLeaderEpoch) {
              ...
              if (isMirrorTopicByLocal(partition.topic) && partitionState.replicas.contains(localBrokerId)) {
                stateChangeLogger.warn(s"LeaderAndIsr request from " +
                  s"controller $controllerId with correlation id $correlationId " +
                  s"epoch $controllerEpoch for partition $topicPartition since its associated " +
                  s"leader epoch $requestLeaderEpoch is smaller than the current " +
                  s"leader epoch $currentLeaderEpoch" +
                  s", but the partition $topicPartition is a mirror topic partition and will still be processed")
                partitionStates.put(partition, partitionState)
              } else {
                stateChangeLogger.warn(s"Ignoring LeaderAndIsr request from " +
                  s"controller $controllerId with correlation id $correlationId " +
              ...
              if (isMirrorTopicByLocal(partition.topic) && partitionState.replicas.contains(localBrokerId)) {
                stateChangeLogger.warn(s"LeaderAndIsr request from " +
                  s"controller $controllerId with correlation id $correlationId " +
                  s"epoch $controllerEpoch for partition $topicPartition since its associated " +
                  s"leader epoch $requestLeaderEpoch is matches the the current " +
                  s"leader epoch $currentLeaderEpoch" +
                  s", but the partition $topicPartition is a mirror topic partition and will still be processed")
                partitionStates.put(partition, partitionState)
              } else {
          ...
          if (logManager.getLog(topicPartition, isFuture = true).isDefined) {
            partition.log.foreach { log =>
              ...
              futureReplicasAndInitialOffset.put(topicPartition, InitialFetchState(leader,
                partition.getLeaderEpoch, partition.futureLog.get.highWatermark))
            }
          }
        }
  ...
  private def makeLeaders(controllerId: Int,
    ...
    try {
      ...
      partitionStates.foreach { case (partition, partitionState) =>
        try {
          if (partition.makeLeader(controllerId, partitionState, correlationId, highWatermarkCheckpoints, isMirrorTopicByLocal(partition.topic))) {
    ...
    // add mirror topic to fecth remote data
  ...
  private def makeFollowers(controllerId: Int,
    ...
    try {
      ...
      removeMirrors(
        //partitionStates.keySet.filter(p => mirrorTopics.contains(p.topic)).map(_.topicPartition),
  ...
  protected def createMirrorFetcherManager(metrics: Metrics, time: Time, threadNamePrefix: Option[String], quotaManager: ReplicationQuotaManager) = {
  ...
  newGauge("MirrorUnkonwTopicCount", () => unknownMirrorTopicLocalNames.size)
  ...
  // unkonw remote topic partition metadata
  private val delayedMakeMirrorPartitions = mutable.Set[TopicPartition]()

  private val remoteClustersNeedReload = mutable.Set[String]()
  ...
  def updateRemoteTopicMetadata(): Unit = {
    ...
    val reloadClustersSnapshot = remoteClustersNeedReload.toSet

    info(s"All mirror fetcher remote partitions ${mirrorFetcherManager.allPartitions() ++ delayedMakeMirrorPartitions}")
    if (mirrorFetcherManager.allFailedPartitions().nonEmpty)
      info(s"All mirror fetcher remote [failed] partitions ${mirrorFetcherManager.allFailedPartitions()}")
    if (delayedMakeMirrorPartitions.nonEmpty)
      info(s"All mirror fetcher remote [delayed] partitions $delayedMakeMirrorPartitions")
    if (unknownMirrorTopicLocalNamesNeedMetadata.nonEmpty)
      info(s"Unknown mirror topics of local topic ${unknownMirrorTopicLocalNamesNeedMetadata.mkString(", ")}")

    // 1. maybe init new cluster client
    val requiresVersion = mutable.HashMap[String, Int]()
    (hostedMirrorTopics ++ unknownMirrorTopics)
      .map(_.remoteCluster)
      .filterNot(remoteClients.contains)
      .foreach {
        remoteCluster => {
          val remoteClientAndMetadata = newRemoteClient(remoteCluster, ClusterConfigManager.getHAClusterConfig(remoteCluster))
...
//    mirrorTopics.values.foreach {
    ...
    // for delay partitons
    // delayedMakeMirrorPartitions unkonw remote topic partition metadata
    (mirrorFetcherManager.allDelayedPartitions() ++ mirrorFetcherManager.allFailedPartitions() ++ delayedMakeMirrorPartitions)
      .flatMap {
        tp =>
    ...
    requiresVersion.foreach {
      case (remoteCluster, lastVersion) =>
        info(s"Requested cluster $remoteCluster metadata last version: $lastVersion")
    }

    // 4. poll send request and receive response
    remoteClients.values.foreach(_.poll(0, nowMs))

    // can use DelayedOperationPurgatory
    // 5. wait metadata
    while ((time.milliseconds() - nowMs < 9 * 1000) && !isShuttingDown.get() && requiresVersion.filter {
      case (remoteCluster, lastVersion) =>
        val metadata = remoteMetadatas(remoteCluster)
        val waitCondition = {
          if (lastVersion == -1) {
            false
          } else {
            ...
            try {
              metadata.maybeThrowFatalException()
              metadata.updateVersion() <= lastVersion
            } catch {
              case e: Exception =>
                warn(s"Failed to update cluster $remoteCluster metadata", e)
                requiresVersion += remoteCluster -> -1
                false
            }
          }
        }
        if (waitCondition) {
          info(s"Wait cluster $remoteCluster metadata current version: ${metadata.updateVersion()} larger than last version: $lastVersion")
    ...
    (mirrorFetcherManager.allPartitions() ++ delayedMakeMirrorPartitions)
      ...
      .foreach {
        case (topicPartition, maybeTopic: Option[MirrorTopic]) =>
          // add reload cluster's partiton to remake mirror
          maybeTopic.filter(t => reloadClustersSnapshot.contains(t.remoteCluster))
            .foreach { _ =>
              partitionsToMakeMirrors += topicPartition
              reopenFetcherThread = true
            }

          maybeTopic.map(t => remoteMetadatas(t.remoteCluster)).map(_.currentLeader(topicPartition)).foreach {
            case leaderAndEpoch if leaderAndEpoch.leader.isPresent =>
              ...
              if (delayedMakeMirrorPartitions.contains(topicPartition)) {
                ...
                (leaderAndEpoch.epoch != oldLeaderAndEpoch.get.epoch || leaderAndEpoch.leader != oldLeaderAndEpoch.get.leader)) {
                // epoch change
                info(s"Will change mirror fetcher for remote partition $topicPartition from ${oldLeaderAndEpoch.get} to $leaderAndEpoch")
                // add mirror fetcher
                partitionsToMakeMirrors += topicPartition
                // update mirror topic leader epoch
                if (!topicPartition.topic().equals(Topic.GROUP_METADATA_TOPIC_NAME)) {
                  leaderAndEpoch.epoch.asScala.foreach{ newEpoch =>
                    info(s"Mirror topic partition $topicPartition remote leader epoch $newEpoch has been changed, " +
                      s"local leader epoch ${oldLeaderAndEpoch.get.epoch} while be increment. By metadata")
                    nonOfflinePartition(topicPartition).foreach(_.expandLeaderEpoch(newEpoch))
                  }
                }
              }
            case _ => None
    ...
    // 7. make mirrors
    makeMirrors(partitionsToMakeMirrors, localStateChange = false, reopenFetcherThread)

    // 8. close idle client and neet reload cluster's client
    val clusters = (hostedMirrorTopics ++ unknownMirrorTopics)
      .map(_.remoteCluster)
    (remoteClients.filterNot(c => clusters.contains(c._1)) ++ remoteClients.filter(c => reloadClustersSnapshot.contains(c._1)))
      .foreach {
      case (clusterId, client) => {
    ...
    // 9. remove need reload cluster
    inLock(mirrorFetcherLock) {
      remoteClustersNeedReload --= reloadClustersSnapshot
    }

    // TODO trace
    info(s"Remote mirror topic leader and epochs $remoteLeaderAndEpochs")
  }
  ...
  def getLocalTopicPartitionByRemoteTopicPartition(remoteTopicPartition: TopicPartition): Option[TopicPartition] = {
    mirrorTopics.get(remoteTopicPartition.topic()).map(mt => new TopicPartition(mt.localTopic, remoteTopicPartition.partition()))
  }
  ...
  def isMirrorTopicByLocal(localTopic: String): Boolean = {
    localTopic match {
      case Topic.GROUP_METADATA_TOPIC_NAME => false
      case _ => mirrorTopicsByLocal.contains(localTopic)
    }
  }

  def isMirrorSyncGroupMetadata(): Boolean = {
    mirrorTopicsByLocal.get(Topic.GROUP_METADATA_TOPIC_NAME)
      .exists(_.syncTopicPartitions)
  }
  ...
  def removeMirrors(partitionsToRemove: Set[TopicPartition], reason: String, isRemoved: Boolean = false): Unit = inLock(mirrorFetcherLock) {
    ...
    partitionsToRemove.foreach { topicPartition =>
      ...
      // Clean mirror state
      nonOfflinePartition(topicPartition).foreach { partition =>
        partition.updateMirrorTopicPartition(false)
        partition.updateRemoteHighWatermark(-1)
        if (isRemoved) {
          // update leader epoch + 1, for consumer failover
          info(s"Update partition $topicPartition leader epoch from ${partition.getLeaderEpoch} to ${partition.getLeaderEpoch + 1}, For consumer failover")
          partition.expandLeaderEpoch(partition.getLeaderEpoch + 1)
        }
      }
    }
    // remove idle connection
    info(s"Shutdown idle fetcher threads due by removeMirrors $partitionsToRemove")
    mirrorFetcherManager.shutdownIdleFetcherThreads()
  }

  def makeMirrors(partitionsToMakeMirrors: Set[TopicPartition], localStateChange: Boolean = true, reopenFetcherThread: Boolean = false): Unit = {
    if (partitionsToMakeMirrors.isEmpty)
      return
    // we do not need to check if the leader exists again since this has been done at the beginning of this process
    val partitionsToMakeMirrorsWithLeaderAndOffset = partitionsToMakeMirrors.flatMap { topicPartition =>
        ...
        case Some(leaderAndEpoch) =>
          ...
          val fetchOffset =
            if (topicPartition.topic().equals(Topic.GROUP_METADATA_TOPIC_NAME))
              ...
              //mirrorFetcherManager.getFetcher(topicPartition).flatMap(_.fetchState(topicPartition)).map(_.fetchOffset).getOrElse(0L)
          ...
          val remoteCluster = mirrorTopics(topicPartition.topic).remoteCluster
          // fetcherThreadMap is host by broker id
          val remoteLeader = BrokerEndPoint(leader.id(), leader.host(), leader.port(), Option(remoteCluster))
    ...
    inLock(mirrorFetcherLock) {
      // check mirrors, maybe removed during the process of metadata, and must brfore remove fetchers
      val allMirrorPartitions = mirrorFetcherManager.allPartitions() ++ delayedMakeMirrorPartitions
      val partitionsToMakeMirrorsWithLeaderAndOffsetWithCheck = partitionsToMakeMirrorsWithLeaderAndOffset.filter(t => allMirrorPartitions.contains(t._1))
      val partitionsToMakeMirrorsWithCheck = partitionsToMakeMirrorsWithLeaderAndOffsetWithCheck.keySet
      // first remove fetcher
      mirrorFetcherManager.removeFetcherForPartitions(partitionsToMakeMirrorsWithCheck)
      partitionsToMakeMirrorsWithCheck.foreach { topicPartition =>
        stateChangeLogger.trace(s"Stopped mirror fetchers of " +
      ...
      // reopen connections
      if (reopenFetcherThread) {
        info(s"Shutdown idle fetcher threads due by makeMirrors $partitionsToMakeMirrorsWithCheck for ha-cluster config changed.")
        mirrorFetcherManager.shutdownIdleFetcherThreads()
      }
      // add fetcher
      mirrorFetcherManager.addFetcherForPartitions(partitionsToMakeMirrorsWithLeaderAndOffsetWithCheck)
      delayedMakeMirrorPartitions --= partitionsToMakeMirrorsWithCheck
      partitionsToMakeMirrorsWithLeaderAndOffsetWithCheck.foreach { case (topicPartition, initialFetchState) =>
        stateChangeLogger.trace(s"Started mirror fetchers of " +
          s"${if (localStateChange) "local state change to" else "remote state change to"} " +
          s"become leader for remote partition $topicPartition with leader ${initialFetchState.leader} and epoch ${initialFetchState.currentLeaderEpoch}")
        // Set mirror state
        nonOfflinePartition(topicPartition).foreach(_.updateMirrorTopicPartition(true))
      }
    }
  }
  ...
  def addMirrorTopics(mirrorTopic: MirrorTopic): Unit = {
    ...
    val partitionsToMakeMirrors = nonOfflinePartitionsIterator
  ...
  def removeMirrorTopicsByLocalTopic(localTopic: String): Unit = {
    mirrorTopicsByLocal.get(localTopic).foreach(m => removeMirrorTopics(m.remoteTopic))
  }
  ...
  def removeMirrorTopics(remoteTopic: String): Unit = {
    ...
    val partitionsToRemove = (mirrorFetcherManager.allPartitions() ++ delayedMakeMirrorPartitions)
      .filter( p => mirrorTopics.contains(p.topic()) && p.topic().equals(remoteTopic))

    removeMirrors(partitionsToRemove, "remote state change to stop mirror", isRemoved = true)
    // remove remove metadata
    remoteLeaderAndEpochs --= remoteLeaderAndEpochs.filter(le => le._1.topic() == remoteTopic).keySet
  ...
  def reloadRemoteCluster(remoteCluster: String): Unit = inLock(mirrorFetcherLock) {
    // 1. remake mirror for this cluster's partiton
    // 2. close and reopen metadata client for this cluster
    remoteClustersNeedReload += remoteCluster
  }
  ...
  private def removeUnknownLocalMirrorTopic(localTopic: String): Unit = {
    ...
    if (unknownMirrorTopicLocalNames.remove(localTopic))
      info(s"Local mirror topic $localTopic has bean created, remove it from unknow list")
  }
  private def addUnknownLocalMirrorTopic(localTopic: String): Unit = {
    info(s"Local mirror topic $localTopic has bean deleted, add it to unknow list")
    unknownMirrorTopicLocalNames.add(localTopic)
  }
