...
class DelayedFetch(delayMs: Long,
                   ...
                   quota: ReplicaQuota,
                   result: Seq[(TopicPartition, FetchPartitionData)] = Seq(),
  ...
  override def onComplete(): Unit = {
    ...
    var fetchPartitionData = logReadResults.map { case (tp, result) =>
      val isAddingReplica = replicaManager.isAddingReplica(tp, fetchMetadata.replicaId)
      val role = if (fetchMetadata.isFromFollower)
        if (isAddingReplica)  -1 else 1
        else 0
      ...
      tp -> FetchPartitionData(result.error, result.highWatermark, result.leaderLogStartOffset, result.info.records,
        ...
        fetchMetadata.isFromFollower && isAddingReplica)
    }
    fetchPartitionData = fetchPartitionData ++ result
