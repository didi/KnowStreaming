  ...
  def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        LoginManager.getInstance().start(clusterId, config.brokerId, zkClient, kafkaGatewayScheduler.getExecutor,
          gatewayUrl, List().asJava)
        SessionManager.getInstance().start(kafkaGatewayScheduler.getExecutor)
