...
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
  ...
  var clientToHAClusterChannelManager: BrokerToHAClusterChannelManager = null
    ...
    try {
      ...
      if (canStartup) {
        ...
        if (_clusterId == "-1") {
          ...
          val cid = zkClient.createOrGetClusterId(_clusterId);
          if (config.clusterIdUpdate && cid !=  _clusterId) {
            zkClient.createOrUpdateClusterId(_clusterId)
          }
        }
        ...
        /* check cluster id */
        if (!config.clusterIdUpdate && preloadedBrokerMetadataCheckpoint.clusterId.isDefined && preloadedBrokerMetadataCheckpoint.clusterId.get != clusterId)
        ...
        kafkaGatewayScheduler = new KafkaScheduler(config.backgroundThreads, "kafka-gateway-schedule")
        kafkaGatewayScheduler.startup()
        ...
        KafkaExMetrics.init(metrics, config.kafkaExMetricsEnableAll, kafkaGatewayScheduler.getExecutor);
        OSUtil.start(config.logDirs.asJava, kafkaGatewayScheduler.getExecutor);
        ...
        clientToHAClusterChannelManager = BrokerToHAClusterChannelManager(
          time = time,
          metrics = metrics,
          config = config,
          channelName = "forwarding-haCluster",
          threadNamePrefix = "forwarding-haCluster")
        clientToHAClusterChannelManager.start();
        /* start forwarding manager */
        forwardingManager = new ForwardingManagerImpl(clientToHAClusterChannelManager)
        ...
        replicaManager.diskLoadProtector = new DiskLoadProtector(kafkaGatewayScheduler.getExecutor, config.diskLoadProtectorEnable)
        ...
        mirrorCoordinator = new MirrorCoordinator(config, metrics, Time.SYSTEM, replicaManager, adminManager, kafkaScheduler, zkClient, authorizer)
        ...
        dataPlaneRequestProcessor = new KafkaApis(socketServer.dataPlaneRequestChannel, replicaManager, adminManager, groupCoordinator, transactionCoordinator,
          ...
          fetchManager, brokerTopicStats, clusterId, time, tokenManager, Option(mirrorCoordinator), Option(forwardingManager))
        ...
        dynamicConfigHandlers = Map[String, ConfigHandler](ConfigType.Topic -> new TopicConfigHandler(logManager, config, quotaManagers, kafkaController),
                                                           ...
                                                           ConfigType.HACluster -> new HaClusterConfigHandler(replicaManager, mirrorCoordinator, clientToHAClusterChannelManager),
                                                           ConfigType.HAGroup -> new HaGroupConfigHandler(),
                                                           ConfigType.HATopic -> new HaTopicConfigHandler(replicaManager),
                                                           ConfigType.DiskLoadProtector-> new CommonConfigHandler(config, replicaManager.diskLoadProtector),
                                                           ConfigType.KafkaExMetrics-> new CommonConfigHandler(config, new KafkaExMetrics()))
  ...
  def shutdown(): Unit = {
    try {
      ...
      if (shutdownLatch.getCount > 0 && isShuttingDown.compareAndSet(false, true)) {
        ...
        if (clientToHAClusterChannelManager != null)
          CoreUtils.swallow(clientToHAClusterChannelManager.shutdown(), this)
