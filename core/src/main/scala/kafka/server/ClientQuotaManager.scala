 ...
 * @param maxThrottleTimeMs The max ThrottleTime
 *
 */
case class ClientQuotaManagerConfig(quotaBytesPerSecondDefault: Long =
                                    ...
                                    maxThrottleTimeMs: Int =
                                        ClientQuotaManagerConfig.DefaultMaxThrottleTimeMs
                                   )
...
object ClientQuotaManagerConfig {
  ...
  val DefaultMaxThrottleTimeMs = 10000
...
class ClientQuotaManager(private val config: ClientQuotaManagerConfig,
  ...
  private val topicQuotaTracer = new ConcurrentHashMap[String, mutable.Map[KafkaQuotaEntity, Double]]()
  private val maxThrottleTimeMs = config.maxThrottleTimeMs
  ...
  def maybeRecordAndGetThrottleTimeMs(request: RequestChannel.Request, value: Double, timeMs: Long): Int = {
    maybeRecordAndGetThrottleTimeMs(request.session, request.throttleId, value, timeMs)
  }
  ...
  def unrecordQuotaSensor(request: RequestChannel.Request, value: Double, timeMs: Long): Unit = {
    val clientSensors = getOrCreateQuotaSensors(request.session, request.throttleId)
  ...
  protected def throttleTime(clientMetric: KafkaMetric): Long = {
    ...
    // throttle time max maxThrottleTimeMs
    val throttleTimeMsRevision = if (throttleTimeMs > maxThrottleTimeMs) maxThrottleTimeMs else throttleTimeMs
    throttleTimeMsRevision.round.toInt
  }
  ...
  def updateQuota(sanitizedUser: Option[String], clientId: Option[String], sanitizedClientId: Option[String], quota: Option[Quota]): Unit = {
    ...
    try {
      ...
      quota match {
        case Some(newQuota) =>
          // 1.add index
          if (clientId.getOrElse("").contains(".")) {
            val topic = clientId.get.split("\\.").toList.last
            if (!topicQuotaTracer.containsKey(topic)) {
              topicQuotaTracer.put(topic, new mutable.HashMap[KafkaQuotaEntity, Double]())
            }
            topicQuotaTracer.get(topic).put(quotaEntity, newQuota.bound)
          }
          quotaCallback.updateQuota(clientQuotaType, quotaEntity, newQuota.bound())
          logger.info(s"Changing (zk) ${quotaType} quota for ${userEntity}${clientIdEntity} to ${newQuota.bound}")
        case None =>
          logger.info(s"Removing ${quotaType} quota for ${userEntity}${clientIdEntity}")
  ...
  def updateTopicQuota(topics: Set[String]): Unit = {
    // topic leader switch，adjust bound
    lock.writeLock().lock()
    try {
      val inValidQuotaIds = new mutable.HashSet[KafkaQuotaEntity]()
      topics.map { topic =>
        val (currentBrokerLeaderNum, totalNum) = MetadataCache.cache.getNumberOfPartition(topic)
        if (currentBrokerLeaderNum > 0) {
          val quotaEntries = topicQuotaTracer.get(topic)
          // delete invalid quotaIds
          if (quotaEntries != null) {
            inValidQuotaIds.clear()
            // adjust bound
            quotaEntries.foreach { quotaId =>
              val quota = quotaCallback.asInstanceOf[DefaultQuotaCallback].getQuota(clientQuotaType, quotaId._1)
              if (quota != null) {
                if (currentBrokerLeaderNum > 0) {
                  val total = quotaId._2
                  val bound = currentBrokerLeaderNum.toDouble / totalNum * total
                  if (bound != quota.bound) {
                    quotaCallback.updateQuota(clientQuotaType, quotaId._1, quota.bound)
                    logger.info(s"Update (topic) ${quotaType} quota for ${quotaId}. ")
                    updateQuotaMetricConfigs(Option(quotaId._1))
                  }
                }
              } else {
                inValidQuotaIds.add(quotaId._1)
              }
            }

            if (inValidQuotaIds.nonEmpty) {
              logger.info(s"Removing quotaId index for ${topic}, removed index detail: ${inValidQuotaIds.toString}")
              quotaEntries --= inValidQuotaIds
            }
          }
        }
      }
  ...
  def getThrottleId(request: RequestChannel.Request, compositionBySize: Map[String, Int]): String = {
    val topic = estimateTopic(compositionBySize)
    if (topicQuotaTracer.containsKey(topic)) {
      val appId = request.session.kafkaSession.getUsername
      appId + "." + topic
    } else {
  ...
  private def estimateTopic(compositionBySize: Map[String, Int]): String = {
    val totalSize = compositionBySize.values.sum
    var randomNum = if (totalSize == 0) 0 else new util.Random().nextInt(totalSize)
    var topic = ""
    var found = false
    for (kv <- compositionBySize) {
      randomNum -= kv._2
      if (!found && randomNum <= 0) {
        topic = kv._1
        found = true
      }
    }
  ...
  class DefaultQuotaCallback extends ClientQuotaCallback {
    ...
    override def updateQuota(quotaType: ClientQuotaType, entity: ClientQuotaEntity, newValue: Double): Unit = {
      ...
      if (quotaEntity.clientId.contains(".")) {
        val topic = quotaEntity.clientId.split("\\.").toList.last
        val quotaEntry = topicQuotaTracer.get(topic)
        if (quotaEntry != null) {
          val (currentBrokerLeaderNum, totalNum) = MetadataCache.cache.getNumberOfPartition(topic)
          if (currentBrokerLeaderNum > 0) {
            val total = quotaEntry(quotaEntity)
            val bound = currentBrokerLeaderNum.toDouble / totalNum * total
            val oldQuotaEntity = overriddenQuotas.get(quotaEntry)
            if (oldQuotaEntity == null || bound != oldQuotaEntity.bound) {
              logger.info(s"Update ${quotaType} quota bound for ${entity} to ${bound}, total is ${total}")
              overriddenQuotas.put(quotaEntity, new Quota(bound, true))
            }
            return
          }
        }
    ...
    def getQuota(quotaType: ClientQuotaType, entity: ClientQuotaEntity): Quota = {
      val quotaEntity = entity.asInstanceOf[KafkaQuotaEntity]
      overriddenQuotas.get(quotaEntity)
    }
