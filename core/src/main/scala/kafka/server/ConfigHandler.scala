...
class UserConfigHandler(private val quotaManagers: QuotaManagers, val credentialProvider: CredentialProvider) extends QuotaConfigHandler(quotaManagers) with ConfigHandler {
  ...
  def processConfigChanges(quotaEntityPath: String, config: Properties): Unit = {
    ...
    userConfigManager.configure(Sanitizer.desanitize(sanitizedUser), config)
...
class BrokerConfigHandler(private val brokerConfig: KafkaConfig,
  ...
  def processConfigChanges(brokerId: String, properties: Properties): Unit = {
    ...
    if (brokerId == ConfigEntityName.Default)
      ...
      quotaManagers.mirrorLeader.updateQuota(upperBound(getOrDefault(DiDiMirrorLeaderThrottledRateProp)))
      quotaManagers.mirrorFollower.updateQuota(upperBound(getOrDefault(DiDiMirrorFollowerThrottledRateProp)))
      quotaManagers.alterLogDirs.updateQuota(upperBound(getOrDefault(ReplicaAlterLogDirsIoMaxBytesPerSecondProp)))
    }
  }
}

class HaClusterConfigHandler(private val replicaManager: ReplicaManager, private val mirrorCoordinator: MirrorCoordinator, private val clientToHAClusterConfigurable: ConfigHandler) extends ConfigHandler with Logging {
  val clusterConfigManager = new ClusterConfigManager(replicaManager, mirrorCoordinator)
  def processConfigChanges(clusterId: String, properties: Properties) {
    clusterConfigManager.configure(clusterId, properties)
    clientToHAClusterConfigurable.processConfigChanges(clusterId, properties)
  }
}

class HaGroupConfigHandler() extends ConfigHandler with Logging {
  val groupConfigManager = new GroupConfigManager()
  def processConfigChanges(grooupId: String, properties: Properties) {
    groupConfigManager.configure(grooupId, properties)
  }
}

class HaTopicConfigHandler(private val replicaManager: ReplicaManager) extends ConfigHandler with Logging {
  val topicConfigManager = new TopicConfigManager(replicaManager)
  def processConfigChanges(grooupId: String, properties: Properties) {
    topicConfigManager.configure(grooupId, properties)
  }
}


class CommonConfigHandler(private val brokerConfig: KafkaConfig, private val configurable: Configurable) extends ConfigHandler with Logging {

  def processConfigChanges(key: String, properties: Properties) {
    if (key.isEmpty || brokerConfig.brokerId == key.trim.toInt) {
      configurable.configure(properties.asInstanceOf[java.util.Map[String,_]])
    }
  }
}
