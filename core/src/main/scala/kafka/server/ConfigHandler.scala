...
/**
 * The GatewayUserConfigHandler will process <user> and <user, client-id> quota changes in ZK.
 * The callback provides the node name containing sanitized user principal, sanitized client-id if this is
 * a <user, client-id> update and the full properties set read from ZK.
 */
class GatewayUserConfigHandler() extends ConfigHandler {
...
class ClusterConfigHandler(discoverer: Discoverer) extends ConfigHandler with Logging {
  ...
  def processConfigChanges(clusterId: String, properties: Properties) {
    ...
    discoverer.initClusterClientFromZK(clusterId)
  }
}
