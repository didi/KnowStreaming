...
abstract class DelayedOperation(override val delayMs: Long,
  ...
  val session: Session = if (RequestChannel.currentSession != Session.UnkonwnSession)
    RequestChannel.currentSession else Session.AdminSession
  // Visible for testing
  ...
  def forceComplete(): Boolean = {
    if (completed.compareAndSet(false, true)) {
      ...
      cancel()
      RequestChannel.runWithOtherSession(session, ()=>
        onComplete()
      )
      true
    } else {
