...
abstract class DelayedOperation(override val delayMs: Long,
  ...
  val session = if (RequestChannel.currentSession != null) RequestChannel.currentSession else Session.AdminSession
  // Visible for testing
  ...
  override def run(): Unit = {
    val success = RequestChannel.setSession(session)
    if (forceComplete())
      onExpiration()
    if (success) RequestChannel.cleanSession
  }
}
