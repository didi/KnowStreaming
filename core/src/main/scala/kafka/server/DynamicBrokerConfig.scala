...
object DynamicThreadPool {
  val ReconfigurableConfigs = Set(
    ...
    KafkaConfig.DiDiMirrorNumFetchersProp,
...
class DynamicThreadPool(server: KafkaServer) extends BrokerReconfigurable {
  ...
  override def reconfigure(oldConfig: KafkaConfig, newConfig: KafkaConfig): Unit = {
    ...
    if (newConfig.mirrorNumFetchers != oldConfig.mirrorNumFetchers)
      server.replicaManager.mirrorFetcherManager.resizeThreadPool(newConfig.mirrorNumFetchers)
  ...
  private def currentValue(name: String): Int = {
    name match {
      ...
      case KafkaConfig.DiDiMirrorNumFetchersProp => server.config.mirrorNumFetchers
