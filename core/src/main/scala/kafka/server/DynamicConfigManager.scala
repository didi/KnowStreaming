...
object ConfigType {
  ...
  val DiskLoadProtector = "DiskLoadProtector"
  val KafkaExMetrics = "KafkaExMetrics"
  val all = Seq(Topic, Client, HACluster, User, Broker, DiskLoadProtector, KafkaExMetrics)
}
...
//  var notifyHandlers = scala.collection.mutable.Buffer[ConfigListener]()
//
//  def addNotifyHandle(listener :ConfigListener) : Unit = {
//    if(!notifyHandlers.contains(listener)) {
//      notifyHandlers.asJava.add(listener)
//    }
//  }
//
//  def removeNotifyHandle(listener:ConfigListener): Unit = {
//    if(notifyHandlers.contains(listener)){
//      notifyHandlers.asJava.remove(listener)
//    }
//  }
...
//        notifyHandlers.foreach(listener => {
//          listener.onConfigChanged(entityType, entity)
//        })
//      }
    }
    ...
    private def processEntityConfigChangeVersion2(jsonBytes: Array[Byte], js: JsonObject): Unit = {
      ...
      var index = -1;
      var rootEntityType = "";
      var fullSanitizedEntityName = "";
      configHandlers.keySet.foreach(
        handlename =>
          if (entityPath.startsWith(handlename)) {
            index = handlename.length
            rootEntityType = entityPath.substring(0, index)
            if (entityPath.length > index)
              fullSanitizedEntityName = entityPath.substring(index+1)
          }
      );
...
//      if (notifyHandlers != null) {
//        notifyHandlers.foreach(listener => {
//          listener.onConfigChanged(rootEntityType, fullSanitizedEntityName)
//        })
//      }
    }
  }
  ...
  def startup(): Unit = {
    ...
    configHandlers.foreach {
      ...
      case (configType, handler) if configType == ConfigType.DiskLoadProtector |
          configType == ConfigType.KafkaExMetrics =>
        var execute = false;
        ...
        if (!execute) {
          val config = adminZkClient.fetchEntityConfig(configType, "");
          if (config != null) {
            handler.processConfigChanges("", config)
          }
        }
