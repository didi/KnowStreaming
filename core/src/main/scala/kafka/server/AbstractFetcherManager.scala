  ...
  def addFetcherForPartitions(partitionAndOffsets: Map[TopicPartition, InitialFetchState]): Unit = {
    lock synchronized {
      ...
      for ((brokerAndFetcherId, initialFetchOffsets) <- partitionsPerFetcher) {
        val brokerIdAndFetcherId = BrokerIdAndFetcherId(brokerAndFetcherId.broker.id, brokerAndFetcherId.fetcherId, brokerAndFetcherId.broker.remoteCluster)
...
case class BrokerIdAndFetcherId(brokerId: Int, fetcherId: Int, remoteCluster: Option[String] = None)
