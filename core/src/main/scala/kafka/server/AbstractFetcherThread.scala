...
import com.didichuxing.datachannel.kafka.security.authorizer.Session
...
abstract class AbstractFetcherThread(name: String,
  ...
  protected def isOffsetForLeaderEpochSupported(topicPartition: TopicPartition): Boolean = {
    isOffsetForLeaderEpochSupported
  }

  protected def isOffsetForLeaderEpochSupported: Boolean = true
    ...
    partitionStates.partitionStateMap.forEach { (tp, state) =>
      if (state.isTruncating) {
        latestEpoch(tp) match {
          case Some(epoch) if isOffsetForLeaderEpochSupported(tp) =>
...
class FetcherStats(metricId: ClientIdAndBroker) extends KafkaMetricsGroup {
  ...
  val requestTime = newHistogram(FetcherMetrics.RequestTimeMs, biased = true, tags);
  val totalTime = newHistogram(FetcherMetrics.TotalTimeMs, biased = true, tags);

  def unregister(): Unit = {
    removeMetric(FetcherMetrics.RequestsPerSec, tags)
    removeMetric(FetcherMetrics.BytesPerSec, tags)
    removeMetric(FetcherMetrics.RequestTimeMs, tags)
    removeMetric(FetcherMetrics.TotalTimeMs, tags)
  }
