...
class BrokerTopicMetrics(name: Option[String]) extends KafkaMetricsGroup {
  ...
  if (name.isEmpty) {
    ...
    metricTypeMap.put(BrokerTopicStats.MirrorBytesInPerSec, MeterWrapper(BrokerTopicStats.MirrorBytesInPerSec, "bytes"))
  ...
  private[server] def mirrorBytesInRate: Option[Meter] =
    if (name.isEmpty) Some(metricTypeMap.get(BrokerTopicStats.MirrorBytesInPerSec).meter())
    else None
...
object BrokerTopicStats {
  ...
  val MirrorBytesInPerSec = "MirrorBytesInPerSec"
...
class BrokerTopicStats {
  ...
  def updateMirrorBytesIn(value: Long): Unit = {
    allTopicsStats.mirrorBytesInRate.foreach { metric =>
      metric.mark(value)
    }
  }
  ...
  def removeOldLeaderMetrics(topic: String): Unit = {
    ...
    if (topicMetrics != null) {
      ...
      topicMetrics.closeMetric(BrokerTopicStats.MirrorBytesInPerSec)
    }
  }
  ...
  def updateBytesOut(topic: String, isFollower: Boolean, isReassignment: Boolean, value: Long): Unit = {
    if (isFollower) {
      ...
      Option(KafkaExMetrics.appIdTopicMetrics(topic)).foreach(_.markBytesOutRate(value))
