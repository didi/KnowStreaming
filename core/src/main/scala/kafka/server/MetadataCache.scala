...
import com.didichuxing.datachannel.kafka.server.TopicPartitionAndReplica
...
class MetadataCache(brokerId: Int) extends Logging {
  ...
  private val maintainTopics = mutable.HashSet[TopicPartitionAndReplica]()

  MetadataCache.cache = this
  ...
  def updateMetadata(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest): Seq[TopicPartition] = {
    inWriteLock(partitionMetadataLock) {
      ...
      maintainTopics.clear()
      val newBrokerIds = new java.util.HashSet[Integer]
      updateMetadataRequest.liveBrokers.asScala.foreach { broker =>
        ...
        if (metadataSnapshot.aliveBrokers.nonEmpty && !metadataSnapshot.aliveBrokers.contains(broker.id) &&
            broker.id != brokerId) {
          newBrokerIds.add(broker.id)
        }
      ...
      if (!updateMetadataRequest.partitionStates.iterator.hasNext) {
        metadataSnapshot.partitionStates.foreach { case (topic, oldPartitionStates) =>
          //new broker online all the replica in this broker are maintain topic
          oldPartitionStates.foreach{
            entry =>
              val info = entry._2
              if (info.leader() == brokerId) {
                info.replicas().asScala.foreach{
                  replicaId =>
                    if (newBrokerIds.contains(replicaId)) {
                      val tp = new TopicPartition(topic, info.partitionIndex)
                      maintainTopics.add(new TopicPartitionAndReplica(tp, replicaId))
                      log4j.info("add maintain topic partition for new brokers added : {}, {}{}", tp, replicaId, "")
                    }
                }
              }
          }
        ...
        updateMetadataRequest.partitionStates.asScala.foreach { info =>
          ...
          if (info.leader == LeaderAndIsr.LeaderDuringDelete) {
            ...
            //new broker online all the replica in this broker are maintain topic
            if (info.leader() == brokerId) {
              info.replicas.asScala.foreach(replicaId => if (newBrokerIds.contains(replicaId)) {
                maintainTopics.add(new TopicPartitionAndReplica(tp, replicaId))
                log4j.info("add maintain topic partition for new brokers added : {}, {}{}", tp, replicaId, "")
              })
            }
            val oldReplicas = getPartitionInfo(tp.topic, tp.partition()).getOrElse(new UpdateMetadataPartitionState()).
              replicas().asScala.toSet
            if (oldReplicas.nonEmpty) {
              val newReplicas = info.replicas().asScala.toSet -- oldReplicas - brokerId;
              newReplicas.foreach { replica =>
                if (info.leader == brokerId) {
                  maintainTopics.add(new TopicPartitionAndReplica(tp, replica))
                  log4j.info("add maintain topic partition for expand replica: {}, {}{}", tp, replica, "");
                }
              }
            }
      ...
      log4j.debug("all maintain topic partitions: {}{}{}", maintainTopics, "", "");
      deletedPartitions
    }
  }
  ...
  def getMaintainTopics:Set[TopicPartitionAndReplica] = {
    inWriteLock(partitionMetadataLock) {
      maintainTopics
    }
  }
