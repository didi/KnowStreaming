...
class Partition(val topicPartition: TopicPartition,
  ...
  // Mirror topic remote HighWatermark
  @volatile private var remoteHighWatermark: Long = -1
  @volatile var isMirrorTopicPartitionLeader: Boolean = false
  ...
  def updateRemoteHighWatermark(highWatermark: Long): Unit = this.remoteHighWatermark = highWatermark

  def updateMirrorTopicPartition(isMirrorTopic: Boolean): Unit = this.isMirrorTopicPartitionLeader = isMirrorTopic
  ...
  private def checkCurrentLeaderEpoch(remoteLeaderEpochOpt: Optional[Integer]): Errors = {
    if (!remoteLeaderEpochOpt.isPresent) {
      Errors.NONE
    } else if (isMirrorTopicPartitionLeader) {
      if (remoteLeaderEpochOpt.get != leaderEpoch) {
        // warn
        debug(s"Get a mirror topic log for " +
          s"partition $topicPartition with leader epoch $remoteLeaderEpochOpt. The current leader " +
  ...
  // update mirror topic follower's leader epoch
  private[kafka] def setLeaderEpoch(newleaderEpoch: Int): Unit = {
  ...
  def makeLeader(controllerId: Int,
                 ...
                 mirrorTopic: Boolean = false): Boolean = {
  ...
  def updateFollowerFetchState(followerId: Int,
    ...
    getReplica(followerId) match {
      ...
      case None if followerId >= FetchRequest.MIRROR_START_REPLICA_ID =>
        true
      case None =>
        false
    }
  ...
  def appendRecordsToLeader(records: MemoryRecords, origin: AppendOrigin, requiredAcks: Int): LogAppendInfo = {
    val (appendInfo, leaderHWIncremented) = inReadLock(leaderIsrUpdateLock) {
      leaderLogIfLocal match {
        case Some(leaderLog) =>
          val minIsr = leaderLog.config.minInSyncReplicas
          ...
          if (origin == AppendOrigin.Mirror) {
            val remoteLeaderEpochs = records.batches.asScala
              .filter(batch => batch.magic >= RecordBatch.MAGIC_VALUE_V2)
              .map(_.partitionLeaderEpoch())
            if (remoteLeaderEpochs.nonEmpty) {
              val remoteLeaderEpoch = remoteLeaderEpochs.max
              if (remoteLeaderEpoch != this.leaderEpoch) {
                info(s"Mirror topic partition $topicPartition remote leader epoch $remoteLeaderEpoch has been changed, " +
                  s"local leader epoch ${this.leaderEpoch} while be increment. By append log")
                this.leaderEpoch = remoteLeaderEpoch
                expandLeaderEpoch(remoteLeaderEpoch)
              }
            }
          }

          val appendInfo = leaderLog.appendAsLeader(records, leaderEpoch = this.leaderEpoch, origin,
            interBrokerProtocolVersion)

          // we may need to increment high watermark since ISR could be down to 1
          (appendInfo, maybeIncrementLeaderHW(leaderLog))
  ...
  def readRecords(fetchOffset: Long,
    ...
    // If topic is mirror topic, return empty records for offset out of range
    if (fetchOffset > initialLogEndOffset && fetchOffset <= this.remoteHighWatermark) {
      throw Errors.NOT_LEADER_FOR_PARTITION.exception(s"Received request for offset $fetchOffset for partition $topicPartition, " +
        s"but we only have log segments in the range $logStartOffset to $initialLogEndOffset and less then remote hw $remoteHighWatermark.")
    }
        ...
        case Left(localLog) =>
          localLog.endOffsetForEpoch(leaderEpoch) match {
            case Some(epochAndOffset) =>
              if (epochAndOffset.offset < this.remoteHighWatermark) {
                info(s"Last offset for mirror topic $topicPartition leader epoch, return remote hw $remoteHighWatermark instead local hw $epochAndOffset")
                new EpochEndOffset(NONE, epochAndOffset.leaderEpoch, this.remoteHighWatermark)
              } else {
  ...
  def expandLeaderEpoch(remoteLeaderEpoch: Int): Unit = {
    val isr = this.inSyncReplicaIds
    val newLeaderAndIsr = new LeaderAndIsr(localBrokerId, remoteLeaderEpoch, isr.toList, zkVersion)
    val zkVersionOpt = stateStore.expandIsr(controllerEpoch, newLeaderAndIsr)
    maybeUpdateIsrAndVersion(isr, zkVersionOpt)
  }
