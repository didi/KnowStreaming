...
object TopicZNode extends Logging {
  ...
  def decode(topic: String, bytes: Array[Byte]): Map[TopicPartition, ReplicaAssignment] = {
    ...
    try {
      ...
      case e : Exception =>
        error(s"Invalid topic data, topic: ${topic}, data: ${new String(bytes)} when decode", e)
        Map.empty
    }
  }
}
...
object ConfigEntityZNode {
  def path(entityType: String, entityName: String) =
    if(entityName.isEmpty) s"${ConfigEntityTypeZNode.path(entityType)}" else s"${ConfigEntityTypeZNode.path(entityType)}/$entityName"
...
object LeaderCandidatesZNode {
  def path = ConfigZNode.path +  "/extension/candidates"
}
object DiskLoadProtectorZNode {
  def path = ConfigZNode.path +  "/extension/disk_load_protector"
}
