...
class KafkaZkClient private[zk] (zooKeeperClient: ZooKeeperClient, isSecure: Boolean, time: Time) extends AutoCloseable with
  ...
  def createOrUpdateData(path: String, data: Array[Byte]): Unit = {

    def set: SetDataResponse = {
      val setDataRequest = SetDataRequest(path, data, ZkVersion.MatchAnyVersion)
      retryRequestUntilConnected(setDataRequest)
    }
    def create: CreateResponse = {
      val createRequest = CreateRequest(path, data, defaultAcls(path), CreateMode.PERSISTENT)
      retryRequestUntilConnected(createRequest)
    }

    val setDataResponse = set
    setDataResponse.resultCode match {
      case Code.NONODE =>
        val createDataResponse = create
        createDataResponse.maybeThrow
      case _ => setDataResponse.maybeThrow
    }
  }
  ...
  /**
   * Update the cluster Id. If the cluster id not exists, create the cluster id.
   * @return  cluster id
   */
  def createOrUpdateClusterId(proposedClusterId: String): Unit = {
    try {
      createOrUpdateData(ClusterIdZNode.path, ClusterIdZNode.toJson(proposedClusterId))
    } catch {
      case _: Exception =>
        throw new KafkaException("Failed to update cluster id from Zookeeper. This can happen if /cluster/id is deleted from Zookeeper.")
    }
  }
