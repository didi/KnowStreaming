...
import com.didichuxing.datachannel.kafka.config.manager.{ClusterConfigManager, GroupConfigManager, TopicConfigManager, UserConfigManager}
  ...
  */
  ...
  def changeConfigs(entityType: String, entityName: String, configs: Properties): Unit = {
    ...
    entityType match {
      ...
      case ConfigType.HACluster => changeHaClusterConfig(entityName, configs)
      case ConfigType.HAGroup => changeHaGroupConfig(entityName, configs)
      case ConfigType.HATopic => changeHaTopicConfig(entityName, configs)
   ...
   *
   */
  def changeHaTopicConfig(topic: String, configs: Properties): Unit = {
    validateHaTopicConfig(configs)
    changeEntityConfig(ConfigType.HATopic, topic, configs)
  }

  def validateHaTopicConfig(configs: Properties): Unit = {
    TopicConfigManager.validateConfigs(configs);
  }
   ...
   *
   */
  def changeHaClusterConfig(cluster: String, configs: Properties): Unit = {
    validateHaClusterConfig(configs)
    changeEntityConfig(ConfigType.HACluster, cluster, configs)
  }

  def validateHaClusterConfig(configs: Properties): Unit = {
    ClusterConfigManager.validateConfigs(configs)
  }

  /**
   * Update the config for an existing group and create a change notification
   *
   * @param group: The group for which configs are being changed
   * @param configs: The final set of configs that will be applied to the group. If any new configs need to be added or
   *                 existing configs need to be deleted, it should be done prior to invoking this API
   *
   */
  def changeHaGroupConfig(group: String, configs: Properties): Unit = {
    validateHaGroupConfig(configs)
    changeEntityConfig(ConfigType.HAGroup, group, configs)
  }

  def validateHaGroupConfig(configs: Properties): Unit = {
    GroupConfigManager.validateConfigs(configs)
  }
  ...
  def validClusterConfigsDeletable(cluster: String): Unit = {
    val allHaTopicConfigs = fetchAllEntityConfigs(ConfigType.HATopic)
    for (topic <- allHaTopicConfigs.keys) {
      val haTopicConfigs = allHaTopicConfigs.get(topic).get
      if (TopicConfigManager.ifConfigsClusterRelated(haTopicConfigs, cluster))
  ...
  def changeEntityConfig(rootEntityType: String, fullSanitizedEntityName: String, configs: Properties): Unit = {
   ...
   * @return
   */
  def getAllUserConfigs(): Map[String, Properties] =
    zkClient.getAllConfiguredUsers.map(user => (user, fetchEntityConfig(ConfigType.User, user))).toMap
