...
object Election {
    ...
    leaderAndIsrOpt match {
      case Some(leaderAndIsr) =>
        ...
        val newLeaderAndIsrOpt = leaderOpt.map { leader =>
          ...
          leaderAndIsr.newLeaderAndIsr(partition, leader, newIsr)
        }
  ...
  private def leaderForReassign(partition: TopicPartition,
    ...
    val newLeaderAndIsrOpt = leaderOpt.map(leader => leaderAndIsr.newLeader(partition, leader))
  ...
  private def leaderForControlledShutdown(partition: TopicPartition,
    ...
    val newLeaderAndIsrOpt = leaderOpt.map(leader => leaderAndIsr.newLeaderAndIsr(partition, leader, newIsr))
