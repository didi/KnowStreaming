...
class KafkaController(val config: KafkaConfig,
  ...
  private def scheduleUpdateControllerMetricsTask(): Unit = {
    kafkaScheduler.schedule("update-controller-metrics-task ", updateMetrics,
  ...
  private def updateLeaderAndIsrCache(partitions: Seq[TopicPartition] = controllerContext.allPartitions.toSeq): Map[TopicPartition, LeaderIsrAndControllerEpoch] = {
    val leaderIsrAndControllerEpochs = zkClient.getTopicPartitionStates(partitions)

    val updatedByMirror = mutable.Map.empty[TopicPartition, LeaderIsrAndControllerEpoch]
    leaderIsrAndControllerEpochs.foreach { case (partition, leaderIsrAndControllerEpoch) =>
      // compare old epoch, normal equals
      controllerContext.partitionLeadershipInfo.get(partition)
        .filter(_.leaderAndIsr.leaderEpoch != leaderIsrAndControllerEpoch.leaderAndIsr.leaderEpoch)
        .foreach { _ =>
          // mirror topic partition leader epoch updated by remote topic
          updatedByMirror.put(partition, leaderIsrAndControllerEpoch)
        }
      // update
    ...
    while (!zkWriteCompleteOrUnnecessary) {
      ...
      zkWriteCompleteOrUnnecessary = zkClient.getTopicPartitionStates(Seq(partition)).get(partition) match {
        case Some(leaderIsrAndControllerEpoch) =>
          ...
          val newLeaderAndIsr = leaderAndIsr.newEpochAndZkVersion(partition)
  ...
  private def elect(): Unit = {
    ...
    try {
      val candidates = zkClient.getChildren(LeaderCandidatesZNode.path).toSet;
      if (candidates.nonEmpty) {
        val exsit = candidates.contains(String.valueOf(config.brokerId))
        if (!exsit) {
          info(config.brokerId + " is not a leader candidate, back off 3s")
          Thread.sleep(3000);
        }
      }
  ...
  private def processIsrChangeNotification(): Unit = {
    ...
    def processLeaderAndIsrUpdateNotifications(updatedLeaderIsrAndControllerEpochs: Map[TopicPartition, LeaderIsrAndControllerEpoch]): Unit = {
      try {
        brokerRequestBatch.newBatch()
        updatedLeaderIsrAndControllerEpochs.foreach { case (partition, updatedLeaderIsrAndControllerEpoch) =>
          val assignment = controllerContext.partitionReplicaAssignment(partition)
          val liveReplicas = assignment.filter(replica => controllerContext.isReplicaOnline(replica, partition))
          // val liveReplicas = updatedLeaderIsrAndControllerEpoch.leaderAndIsr.isr
          debug(s"Sending LeaderAndIsrRequest to Brokers: $liveReplicas for TopicPartitions: $partition, Maybe by remote leader epoch: $updatedLeaderIsrAndControllerEpoch updated")
          brokerRequestBatch.addLeaderAndIsrRequestForBrokers(liveReplicas, partition,
            updatedLeaderIsrAndControllerEpoch, controllerContext.partitionFullReplicaAssignment(partition), isNew = false)
        }
        brokerRequestBatch.sendRequestsToBrokers(controllerContext.epoch)
      } catch {
        case e: IllegalStateException =>
    ...
    try {
      ...
      if (partitions.nonEmpty) {
        val partitionsLeaderAndIsrUpdate = updateLeaderAndIsrCache(partitions)
        val partitionsToUpdate = mutable.Set[TopicPartition]()
        partitionsToUpdate ++= partitions
        partitionsToUpdate --= partitionsLeaderAndIsrUpdate.keys
        if (partitionsToUpdate.nonEmpty) {
          processUpdateNotifications(partitionsToUpdate.toSeq)
        }
        if (partitionsLeaderAndIsrUpdate.nonEmpty) {
          processLeaderAndIsrUpdateNotifications(partitionsLeaderAndIsrUpdate)
        }
      }
    } finally {
