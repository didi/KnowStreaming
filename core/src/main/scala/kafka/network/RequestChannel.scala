...
import com.didichuxing.datachannel.kafka.metrics.{KafkaExMetrics, TopicRequestMetrics}
import com.typesafe.scalalogging.Logger
import com.yammer.metrics.core.Meter
import kafka.log.LogConfig
import kafka.metrics.KafkaMetricsGroup
import kafka.server.KafkaConfig
import kafka.utils.{Logging, NotNothing, Pool, threadsafe}
import org.apache.kafka.common.config.types.Password
import org.apache.kafka.common.config.ConfigResource
import org.apache.kafka.common.memory.MemoryPool
import org.apache.kafka.common.message.{EnvelopeResponseData, IncrementalAlterConfigsRequestData}
...
import com.didichuxing.datachannel.kafka.security.authorizer.{SessionManager, Session => KafkaSession}
...
object RequestChannel extends Logging {
  ...
  val MaxNumRequestQueue = 8;

  def isRequestLoggingEnabled: Boolean = requestLogger.underlying.isDebugEnabled

  private val threadSession : ThreadLocal[KafkaSession] =
    new ThreadLocal[KafkaSession];
  def currentSession : KafkaSession = {
    var session = threadSession.get()
    if (session == null) {
      session = KafkaSession.UnkonwnSession
      warn("thread session is null, use unknown session")
    }
    session
  }

  def setSession(session: KafkaSession) : Boolean = {
    if (session == null) {
      throw new Exception("set session should not be null")
    }
    val oldSession = threadSession.get()
  ...
  def runWithOtherSession(otherSession: KafkaSession, func: () => Unit ): Unit = {
    if (otherSession != this.currentSession) {
  ...
  case class Session(principal: KafkaPrincipal, clientAddress: InetAddress, clientPort: Int = -1) {
    val sanitizedUser: String = Sanitizer.sanitize(principal.getName)
    var kafkaSession : KafkaSession = SessionManager.getInstance().getSession(this)
  }
  ...
  class Request(val processor: Int,
    ...
    var topicRequestSizeInByes : mutable.HashMap[String, Long] = mutable.HashMap()
    var topicResponseSizeInByes : mutable.HashMap[String, Long] = mutable.HashMap()
    var topicMessageConversionsTime: mutable.HashMap[String, Long] = mutable.HashMap()
    var topicTemporaryMemoryBytes: mutable.HashMap[String, Long] = mutable.HashMap()

    var throttleId: String = header.clientId
    val session = Session(context.principal, context.clientAddress, context.clientPort)
    private val bodyAndSize: RequestAndSize = context.parseRequest(buffer)

    def header: RequestHeader = context.header
    def sizeOfBodyInBytes: Int = bodyAndSize.size
    def sizeOfResponseInBytes(response:Response) : Long = {
      response match {
        case resp: SendResponse =>
          resp.responseSend.size();
        case _ =>
          0
      }
    ...
    def initTopicMetrics(topic:String): Unit = {
      topicRequestSizeInByes.getOrElseUpdate(topic, 0)
      topicResponseSizeInByes.getOrElseUpdate(topic, 0)
      topicMessageConversionsTime.getOrElseUpdate(topic, 0)
      topicTemporaryMemoryBytes.getOrElseUpdate(topic, 0)
    }
    ...
    def updateRequestMetrics(networkThreadTimeNanos: Long, response: Response): Unit = {
      ...
      metricNames.foreach { metricName =>
        ...
        m.responseBytesHist.update(sizeOfResponseInBytes(response))
        m.messageConversionsTimeHist.foreach(_.update(Math.round(messageConversionsTimeMs)))
        m.tempMemoryBytesHist.foreach(_.update(temporaryMemoryBytes))
      }

      def updateTopicRequestMetics(topicRequestMetrics: TopicRequestMetrics, topic: String): Unit = {
        topicRequestMetrics.markRequest(header.apiVersion())
        topicRequestMetrics.requestQueueTimeHist.update(Math.round(requestQueueTimeMs))
        topicRequestMetrics.localTimeHist.update(Math.round(apiLocalTimeMs))
        topicRequestMetrics.remoteTimeHist.update(Math.round(apiRemoteTimeMs))
        topicRequestMetrics.throttleTimeHist.update(Math.round(apiThrottleTimeMs))
        topicRequestMetrics.responseQueueTimeHist.update(Math.round(responseQueueTimeMs))
        topicRequestMetrics.responseSendTimeHist.update(Math.round(responseSendTimeMs))
        topicRequestMetrics.totalTimeHist.update(Math.round(totalTimeMs))
        topicRequestMetrics.requestBytesHist.update(topicRequestSizeInByes(topic))
        topicRequestMetrics.responseBytesHist.update(topicResponseSizeInByes(topic))
        topicRequestMetrics.messageConversionsTimeHist.update(Math.round(topicMessageConversionsTime(topic)))
        topicRequestMetrics.tempMemoryBytesHist.update(Math.round(topicTemporaryMemoryBytes(topic)))
      }
      ...
      header.apiKey match {
        case ApiKeys.FETCH =>
          val isFromFollower = body[FetchRequest].isFromFollower
          if (!isFromFollower) {
            topicRequestSizeInByes.foreach { entry =>
              Option(KafkaExMetrics.fetchRequestMetrics(entry._1)).foreach(updateTopicRequestMetics(_, entry._1))
            }
          }
          session.kafkaSession.setClientVersionByApi(header.apiKey().id, header.apiVersion())
          session.kafkaSession.setClientId(header.clientId());
        case ApiKeys.PRODUCE =>
          topicRequestSizeInByes.foreach{ entry=>
            Option(KafkaExMetrics.produceRequestMetrics(entry._1)).foreach(updateTopicRequestMetics(_, entry._1))
          }
          session.kafkaSession.setClientVersionByApi(header.apiKey().id, header.apiVersion())
          session.kafkaSession.setClientId(header.clientId());
        case _ =>
          // session.kafkaSession.setClientVersion(context.clientInformation.softwareVersion());
          // session.kafkaSession.setClientId(header.clientId());
      }
...
class RequestChannel(val queueSize: Int, val metricNamePrefix : String) extends KafkaMetricsGroup {
  ...
  var numRequestQueue = 1
  var numPollProcessors = 0;
  private val requestQueues = new Array[ArrayBlockingQueue[BaseRequest]](MaxNumRequestQueue)
  private val processors = new ConcurrentHashMap[Int, Processor]()
  val requestQueueSizeMetricName = metricNamePrefix.concat(RequestQueueSizeMetric)
  val responseQueueSizeMetricName = metricNamePrefix.concat(ResponseQueueSizeMetric)

  newGauge(requestQueueSizeMetricName, () => requestQueues.map(_.size).sum)
  ...
  for(i <- 0 until MaxNumRequestQueue) {
    requestQueues(i) = new ArrayBlockingQueue[BaseRequest](queueSize)
  }
  ...
  @threadsafe
  def addReceiveProcessor(): Unit = {
    numPollProcessors = numPollProcessors + 1
    resizeRequestQueue()
  }

  @threadsafe
  def removeReceiveProcessor(): Unit = {
    numPollProcessors = Math.max(numPollProcessors - 1, 1)
    resizeRequestQueue()
  }

  def resizeRequestQueue(): Unit = {
    var newRequestQueueSize = Math.min(processors.size/12, numPollProcessors)
    newRequestQueueSize = Math.min(newRequestQueueSize, MaxNumRequestQueue)
    newRequestQueueSize = if (newRequestQueueSize == 0) 1 else newRequestQueueSize
    if (numRequestQueue == newRequestQueueSize)
      return
    if (numRequestQueue < newRequestQueueSize) {
      numRequestQueue = newRequestQueueSize
    } else {
      val numQueueForClear = numRequestQueue - newRequestQueueSize;
      numRequestQueue = newRequestQueueSize
      for (i <- 0 to numQueueForClear) {
        val index = numRequestQueue + i;
        requestQueues(0).addAll(requestQueues(index))
        requestQueues(index).clear()
      }
    }
  }
  ...
  def sendRequest(request: RequestChannel.Request): Unit = {
    requestQueues((Thread.currentThread.hashCode % numRequestQueue)).put(request)
  }
  ...
  def receiveRequest(timeout: Long): RequestChannel.BaseRequest =
    requestQueues((Thread.currentThread.hashCode % numRequestQueue)).poll(timeout, TimeUnit.MILLISECONDS)

  /** Get the next request or block until there is one */
  def receiveRequest(): RequestChannel.BaseRequest =
    requestQueues((Thread.currentThread.hashCode % numRequestQueue)).take()
  ...
  def sendShutdownRequest(): Unit = requestQueues.foreach(_.put(ShutdownRequest))
