...
object RequestChannel extends Logging {
  ...
  def setSession(session: KafkaSession) : Boolean = {
    ...
    if (oldSession != null) {
      warn("thread session already exist")
    }
  ...
  def cleanSession(): Unit = {
    ...
    if (session == null) {
      //throw new Exception("clean session should not be null")
      warn("thread session should not be null")
    }
    ...
    def updateRequestMetrics(networkThreadTimeNanos: Long, response: Response): Unit = {
      ...
      header.apiKey match {
        case ApiKeys.FETCH =>
          ...
          session.kafkaSession.setClientVersion(header.apiKey().id, header.apiVersion())
