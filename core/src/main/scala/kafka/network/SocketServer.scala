...
class SocketServer(val config: KafkaConfig,
  ...
  def boundPort(listenerName: ListenerName): Int = {
    try {
      ...
      case e: Exception =>
        throw new KafkaException("Tried to check server's port before server waschange notification must have started or checked for port of non-existing protocol", e)
    }
  }
