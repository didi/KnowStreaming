...
class SocketServer(val config: KafkaConfig,
  ...
  private def createDataPlaneAcceptorsAndProcessors(dataProcessorsPerListener: Int,
    ...
    endpoints.foreach { endpoint =>
      ...
      KafkaThread.nonDaemon(s"kafka-dp-socket-acceptor-${endpoint.listenerName}-${endpoint.securityProtocol}-${endpoint.port}", dataPlaneAcceptor).start()
  ...
  private def createControlPlaneAcceptorAndProcessor(endpointOpt: Option[EndPoint]): Unit = synchronized {
    endpointOpt.foreach { endpoint =>
      ...
      KafkaThread.nonDaemon(s"${ControlPlaneThreadPrefix}-socket-acceptor-${endpoint.listenerName}-${endpoint.securityProtocol}-${endpoint.port}", controlPlaneAcceptor).start()
...
object SocketServer {
  ...
  val DataPlaneThreadPrefix = "kafka-dp"
  val ControlPlaneThreadPrefix = "kafka-cp"
...
private[kafka] class Acceptor(val endPoint: EndPoint,
  ...
  private def startProcessors(processors: Seq[Processor], processorThreadPrefix: String): Unit = synchronized {
    processors.foreach { processor =>
      KafkaThread.nonDaemon(s"${processorThreadPrefix}-network-thread-$brokerId-${endPoint.listenerName}-${endPoint.securityProtocol}-${processor.id}",
        processor).start()
    }
  }
...
private[kafka] class Processor(val id: Int,
          ...
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive, nowNanosSupplier))
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {
                ...
                val context = new RequestContext(header, connectionId, channel.socketAddress, channel.socketPort(),
  ...
  private def updateRequestMetrics(response: RequestChannel.Response): Unit = {
    ...
    val succuss = RequestChannel.setSession(response.request.session.kafkaSession)
    request.updateRequestMetrics(networkThreadTimeNanos, response)
    if (succuss) RequestChannel.cleanSession
  }
...
class ConnectionQuotas(config: KafkaConfig, time: Time, metrics: Metrics) extends Logging {
  ...
  def inc(listenerName: ListenerName, address: InetAddress, acceptorBlockedPercentMeter: com.yammer.metrics.core.Meter): Unit = {
    counts.synchronized {
      val sensorName = address.getHostAddress
      var ipCountSensor = metrics.getSensor(sensorName)
      if (ipCountSensor == null) {
        ipCountSensor = metrics.sensor(sensorName, new MetricConfig, Long.MaxValue)
        ipCountSensor.add(metrics.metricName("ip-rate", "ConnectionQuotas", "Tracking connect-rate per ip", "ip", sensorName), new Value())
      }
      waitForConnectionSlot(listenerName, acceptorBlockedPercentMeter)

      val count = counts.getOrElseUpdate(address, 0)
      val newCount = count + 1
      counts.put(address, newCount)
      ipCountSensor.record(newCount)
