...
class SimpleFetchTest {
  ...
  def setUp(): Unit = {
    ...
    val diskLoadProtector:DiskLoadProtector = EasyMock.createMock(classOf[DiskLoadProtector])
    replicaManager.diskLoadProtector = diskLoadProtector
    EasyMock.expect(diskLoadProtector.checkRead(EasyMock.anyObject())).andAnswer{() => EasyMock.getCurrentArgument(0)}.anyTimes()
    EasyMock.replay(diskLoadProtector)
