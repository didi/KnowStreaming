...
import org.easymock.EasyMock.{anyObject, createMock, expect, getCurrentArgument, replay}
...
class ReplicaManagerTest {
  ...
  private def setupReplicaManagerWithMockedPurgatories(timer: MockTimer, aliveBrokerIds: Seq[Int] = Seq(0, 1)): ReplicaManager = {
    ...
    val diskLoadProtector:DiskLoadProtector = createMock(classOf[DiskLoadProtector])
    replicaManager.diskLoadProtector = diskLoadProtector
    expect(diskLoadProtector.checkRead(anyObject())).andAnswer{() => getCurrentArgument(0)}.anyTimes()
    replay(diskLoadProtector)
    replicaManager
  }
