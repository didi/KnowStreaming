...
class DynamicBrokerReconfigurationTest extends ZooKeeperTestHarness with SaslSetup {
  ...
  def testThreadPoolResize(): Unit = {
    val requestHandlerPrefix = "kafka-dp-request-handler-"
    val networkThreadPrefix = "kafka-dp-network-thread-"
    ...
    verifyThreads("kafka-dp-socket-acceptor-", config.listeners.size)
