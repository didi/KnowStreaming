    ...
    private void handleProduceResponse(ClientResponse response, Map<TopicPartition, ProducerBatch> batches, long now) {
        ...
        if (response.wasDisconnected()) {
            ...
            if (response.hasResponse()) {
                ...
                long throttleTimeMs = produceResponse.throttleTimeMs();
                for (Map.Entry<TopicPartition, ProduceResponse.PartitionResponse> entry : produceResponse.responses().entrySet()) {
                    TopicPartition tp = entry.getKey();
                    ProduceResponse.PartitionResponse partResp = entry.getValue();
                    ProducerBatch batch = batches.get(tp);
                    completeBatch(batch, partResp, correlationId, now, receivedTimeMs + throttleTimeMs);
                }
                if (throttleTimeMs > 0) {
                    Cluster cluster = metadata.fetch();
                    for (Map.Entry<TopicPartition, List<ProducerBatch>> entry : inFlightBatches.entrySet()) {
                        Node leader = cluster.leaderFor(entry.getKey());
                        if (leader != null && leader.idString().equals(response.destination())) {
                            for (ProducerBatch producerBatch : entry.getValue()) {
                                producerBatch.updateThrottleMs(throttleTimeMs);
                            }
                        }
                    }
                    for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : accumulator.batches().entrySet()) {
                        Node leader = cluster.leaderFor(entry.getKey());
                        if (leader != null && leader.idString().equals(response.destination())) {
                            for (ProducerBatch producerBatch : entry.getValue()) {
                                producerBatch.updateThrottleMs(throttleTimeMs);
