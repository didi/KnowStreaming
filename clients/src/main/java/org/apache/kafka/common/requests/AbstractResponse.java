...
public abstract class AbstractResponse implements AbstractRequestResponse {
    ...
    public static AbstractResponse parseResponse(ByteBuffer buffer, RequestHeader requestHeader) {
        ...
        switch (apiKey) {
            case ENVELOPE:
                return EnvelopeResponse.parse(buffer, apiVersion);
            case API_VERSIONS:
                return ApiVersionsResponse.parse(buffer, apiVersion);
            case METADATA:
                return MetadataResponse.parse(buffer, apiVersion);
            case FIND_COORDINATOR:
                return FindCoordinatorResponse.parse(buffer, apiVersion);
            case INIT_PRODUCER_ID:
                return InitProducerIdResponse.parse(buffer, apiVersion);
            default:
    ...
    public static AbstractResponse parseResponse(ApiKeys apiKey, Struct struct, short version) {
        switch (apiKey) {
            ...
            case ENVELOPE:
                return new EnvelopeResponse(struct, version);
            default:
                throw new AssertionError(String.format("ApiKey %s is not currently handled in `parseResponse`, the " +
                        "code should be updated to do so.", apiKey));
        }
