...
public class SaslChannelBuilder implements ChannelBuilder, ListenerReconfigurable {
    ...
    static final String SASL_CHECK_HOSTNAME_ENABLE_PROP = "sasl.check.hostname.enable";
    ...
    private boolean saslCheckHostnameEnable = true;
    ...
    public void configure(Map<String, ?> configs) throws KafkaException {
        try {
            ...
            Boolean saslCheckHostnameEnable = (Boolean) configs.get(SASL_CHECK_HOSTNAME_ENABLE_PROP);
            if (saslCheckHostnameEnable != null) {
                log.info("Set sasl check hostname enable to {}", saslCheckHostnameEnable);
                this.saslCheckHostnameEnable = saslCheckHostnameEnable;
            }
        } catch (Throwable e) {
            close();
        ...
        try {
            ...
            if (mode == Mode.SERVER) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Build channel with checkout host {} and server host is {}", this.saslCheckHostnameEnable, this.saslCheckHostnameEnable ? socket.getInetAddress().getHostName() : socket.getInetAddress().getHostAddress());
                }
                authenticatorCreator = () -> buildClientAuthenticator(configs,
                        saslCallbackHandlers.get(clientSaslMechanism),
                        id,
                        this.saslCheckHostnameEnable ? socket.getInetAddress().getHostName() : socket.getInetAddress().getHostAddress(),
