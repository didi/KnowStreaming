...
public class SaslServerAuthenticator implements Authenticator {
    ...
    private void createSaslServer(String mechanism) throws IOException {
        ...
        serverConfig.put("client_ip", transportLayer.socketChannel().socket().getInetAddress().getHostAddress());
        serverConfig.put("client_port", String.valueOf(transportLayer.socketChannel().socket().getPort()));
        ...
        if (mechanism.equals(SaslConfigs.GSSAPI_MECHANISM)) {
            saslServer = createSaslKerberosServer(callbackHandler, serverConfig, subject);
        } else {
            try {
                saslServer = Subject.doAs(subject, (PrivilegedExceptionAction<SaslServer>) () ->
                    Sasl.createSaslServer(saslMechanism, "kafka", serverAddress().getHostName(), serverConfig, callbackHandler));
