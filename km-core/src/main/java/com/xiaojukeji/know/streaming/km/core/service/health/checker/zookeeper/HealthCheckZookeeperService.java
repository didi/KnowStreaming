...
public class HealthCheckZookeeperService extends AbstractHealthCheckService {
    ...
    private HealthCheckResult checkOutstandingRequests(Tuple<ClusterParam, BaseClusterHealthConfig> singleConfigSimpleTuple) {
        ...
        if (metricsResult.failed() || !metricsResult.hasData()) {
            log.error(
                    "method=checkOutstandingRequests||clusterPhyId={}||param={}||config={}||result={}||errMsg=get metrics failed, may be collect failed or zk mntr command not in whitelist.",clusterPhyId ,param, valueConfig, metricsResult
            );
            return null;
        }
    ...
    private HealthCheckResult checkAliveConnections(Tuple<ClusterParam, BaseClusterHealthConfig> singleConfigSimpleTuple) {
        ...
        if (metricsResult.failed() || !metricsResult.hasData()) {
            log.error(
                    "method=checkAliveConnections||param={}||config={}||result={}||errMsg=get metrics failed, may be collect failed or zk mntr command not in whitelist.",
                    param, valueConfig, metricsResult
            );
            return null;
        }
    ...
    private HealthCheckResult checkApproximateDataSize(Tuple<ClusterParam, BaseClusterHealthConfig> singleConfigSimpleTuple) {
        ...
        if (metricsResult.failed() || !metricsResult.hasData()) {
            log.error(
                    "method=checkApproximateDataSize||param={}||config={}||result={}||errMsg=get metrics failed, may be collect failed or zk mntr command not in whitelist.",
                    param, valueConfig, metricsResult
            );
            return null;
        }
    ...
    private HealthCheckResult checkSentRate(Tuple<ClusterParam, BaseClusterHealthConfig> singleConfigSimpleTuple) {
        ...
        if (metricsResult.failed() || !metricsResult.hasData()) {
            log.error(
                    "method=checkSentRate||param={}||config={}||result={}||errMsg=get metrics failed, may be collect failed or zk mntr command not in whitelist.",
                    param, valueConfig, metricsResult
            );
            return null;
        }
